...
const keyFor = (operation: string, key: string) => `fiteatsy:delegated:idempotency:${operation}:${key}`;

export const executeDelegatedIdempotently = async <T>(input: {
  operation: string;
  key: string;
  execute: () => Promise<T>;
  ...
  if (isTestRuntime()) {
    ...
    if (existing && existing.expiresAt > Date.now()) return { replayed: true, value: existing.value as T };
    const value = await input.execute();
    memory.set(key, { expiresAt: Date.now() + ttlSeconds * 1000, value });
    return { replayed: false, value };
  }
  if (!env.redisUrl) throw new Error('REDIS_URL is required for delegated idempotency.');
  ...
  if (claimed !== 'OK') {
    ...
    if (prior) {
      const parsed = JSON.parse(String(prior)) as { state?: string; value?: T };
      if (parsed.state === 'complete') return { replayed: true, value: parsed.value as T };
    }
