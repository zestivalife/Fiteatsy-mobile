import { HealthProfileRecord, HealthProfileSectionKey, NutritionProfileRecord } from './platform.types.js';
...
export const calculateAgeFromDob = (dobISO: string | null) => {
...
export const calculateBmi = (heightCm: number | null, weightKg: number | null) => {
  if (!heightCm || !weightKg || heightCm <= 0 || weightKg <= 0) return null;
  const heightM = heightCm / 100;
  return Number((weightKg / (heightM * heightM)).toFixed(1));
};

export const calculateWaistToHeightRatio = (waistCm: number | null, heightCm: number | null) => {
  if (!waistCm || !heightCm || waistCm <= 0 || heightCm <= 0) return null;
  return Number((waistCm / heightCm).toFixed(2));
};

export const calculateNutritionProfileCompletion = (
  profile: HealthProfileRecord,
  reportCount: number
): Pick<NutritionProfileRecord, 'completionPercent' | 'readinessScore' | 'aiReady' | 'missingFields' | 'sectionScores'> => {
  const sections = [
