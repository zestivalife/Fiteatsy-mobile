...
const healthProfilePatchSchema = z.object({
  ...
  armCircumferenceCm: z.number().min(10).max(100).optional(),
  thighCircumferenceCm: z.number().min(20).max(150).optional(),
  calfCircumferenceCm: z.number().min(10).max(100).optional(),
  bodyFatPct: z.number().min(2).max(75).optional(),
  bodyFatSource: z.enum(['MANUAL', 'CALCULATED', 'APPLE_HEALTH', 'HEALTH_CONNECT', 'SMART_SCALE', 'CONSULTANT', 'OTHER']).optional(),
  bodyFatMeasuredAt: z.string().datetime().optional(),
  muscleMassKg: z.number().min(1).max(250).optional(),
  muscleMassCategory: z.string().trim().max(80).optional(),
  ...
  wellnessGoalIds: z.array(z.string().trim().regex(/^[a-z0-9_]+$/)).optional(),
