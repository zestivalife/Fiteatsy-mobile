...
export type MedicationExceptionType =
  | 'REPEATED_MISSED_DOSES'
  | 'LOW_7_DAY_ADHERENCE'
  | 'ADHERENCE_DROP'
  | 'CONSECUTIVE_UNRESOLVED_DOSES';

export type MedicationExceptionSeverity = 'INFO' | 'WATCH' | 'ATTENTION';
export type MedicationExceptionStatus = 'OPEN' | 'ACKNOWLEDGED' | 'RESOLVED';

export type MedicationExceptionInput = {
  clientId: string;
  userId: string;
  type: MedicationExceptionType;
  severity: MedicationExceptionSeverity;
  ruleVersion: string;
  title: string;
  summary: string;
...
export type MedicationExceptionRecord = MedicationExceptionInput & {
  id: string;
  status: MedicationExceptionStatus;
...
const mapException = (row: Record<string, unknown>): MedicationExceptionRecord => ({
  id: String(row.id),
  clientId: String(row.client_id),
  userId: String(row.user_id),
  type: String(row.exception_type) as MedicationExceptionType,
  severity: String(row.severity) as MedicationExceptionSeverity,
  status: String(row.status) as MedicationExceptionStatus,
...
export const listActiveMedicationExceptionsForOwner = async (owner: { accountId: string; clientId: string }) => {
...
export const listMedicationExceptionsForOwner = async (owner: { accountId: string; clientId: string }) => {
  const result = await pool.query(
    `
      select *
      from client_medication_exceptions
...
export const getMedicationExceptionById = async (exceptionId: string) => {
  const result = await pool.query(
    `
      select *
      from client_medication_exceptions
...
export const upsertActiveMedicationException = async (input: MedicationExceptionInput) => {
...
export const resolveInactiveMedicationExceptions = async (
  owner: { accountId: string; clientId: string },
  activeTypes: MedicationExceptionType[],
  ruleVersion: string
) => {
  const result = await pool.query(
    `
...
export const acknowledgeMedicationException = async (exceptionId: string, acknowledgedByUserId: string) => {
  const result = await pool.query(
    `
      update client_medication_exceptions
  ...
  if (result.rows[0]) return mapException(result.rows[0]);
