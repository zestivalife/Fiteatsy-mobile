...
export const QA_ADMIN_HANDOFF_PURPOSE = 'qa_admin_session_handoff' as const;
...
type HandoffErrorCode =
  ...
  | 'QA_HANDOFF_EXPIRED'
  | 'QA_HANDOFF_REPLAYED'
  | 'QA_HANDOFF_RATE_LIMITED';

const handoffError = (code: HandoffErrorCode, status: number, message: string, retryAfterSec?: number) =>
  Object.assign(new Error(message), { code, status, retryAfterSec });

const digest = (code: string) => crypto.createHash('sha256').update(code).digest('hex');

const audit = async (client: PoolClient, input: {
  ...
  await client.query(
    `insert into qa_provisioning_audit_events
     ...
     values ($1, null, $2, $3, 'QA_TEST', 'admin', $4, $5::jsonb)`,
    [crypto.randomUUID(), input.targetUserId ?? null, input.action, input.reason,
      JSON.stringify({ ...(input.metadata ?? {}), ...(input.actorReference ? { delegatedActorReference: input.actorReference } : {}) })]
  );
};

const isTestRuntime = () =>
  env.environment.toLowerCase() === 'test' || process.execArgv.some((arg) => arg.includes('--test'));

export const assertQaHandoffExchangeRate = async (key: string) => {
  if (!isTestRuntime()) {
    ...
    if (count === 1) await redisCommand(['EXPIRE', redisKey, String(Math.ceil(HANDOFF_RATE_WINDOW_MS / 1000))]);
    if (count > HANDOFF_RATE_MAX) {
      throw handoffError(
        'QA_HANDOFF_RATE_LIMITED',
        429,
        'Too many handoff attempts. Please try again later.',
        Math.max(1, Math.ceil(HANDOFF_RATE_WINDOW_MS / 1000))
      );
    }
    return;
  }
...
export const issueQaAdminSessionHandoff = async (input: {
  ...
  try {
    ...
    if (!target.rowCount) throw handoffError('QA_HANDOFF_DENIED', 404, 'Eligible QA Admin was not found.');
    const revoked = await client.query(
      `update qa_admin_session_handoffs
          set status = 'revoked'
        where target_user_id = $1 and purpose = $2 and status = 'pending'
        returning id, audit_reference`,
      [input.targetUserId, QA_ADMIN_HANDOFF_PURPOSE]
    );
    ...
    await client.query(
      `insert into qa_admin_session_handoffs
        (id, code_digest, target_user_id, purpose, status, actor_reference, audit_reference, expires_at)
       values ($1, $2, $3, $4, 'pending', $5, $6, $7)`,
      [handoffId, digest(code), input.targetUserId, QA_ADMIN_HANDOFF_PURPOSE, input.actorReference, auditReference, expiresAt.toISOString()]
    );
    await audit(client, {
      actorReference: input.actorReference,
      targetUserId: input.targetUserId,
...
export const exchangeQaAdminSessionHandoff = async (input: {
  code: string;
  targetUserId: string;
  purpose: typeof QA_ADMIN_HANDOFF_PURPOSE;
  ...
  try {
    ...
    const result = await client.query(
      `select h.id, h.target_user_id, h.purpose, h.status, h.actor_reference,
    ...
    await client.query(
      `update qa_admin_session_handoffs set status = 'consumed', consumed_at = now()
