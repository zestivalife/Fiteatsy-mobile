...
export const QA_DIET_HYDRATION_PURPOSE = 'DIET_PARTIAL_PLAN_HYDRATION_E2E' as const;
export type QaBrowserRole = 'consultant' | 'senior_consultant';
const HANDOFF_TTL_MS = 3 * 60 * 1000;
const SESSION_TTL_MS = 30 * 60 * 1000;
const hash = (value: string) => crypto.createHash('sha256').update(value).digest('hex');
const failure = (code: string, status = 401) => Object.assign(new Error('The QA browser handoff is invalid.'), { code, status });

const audit = (client: PoolClient, action: string, targetUserId: string | null, role: string | null, metadata: Record<string, unknown>) =>
  client.query(`insert into qa_provisioning_audit_events
...
export const issueQaBrowserHandoff = async (input: { fixtureSetId: string; identityId: string; role: QaBrowserRole; actorId: string }) => {
  ...
  try {
    ...
    if (!eligible.rowCount) throw failure('QA_HANDOFF_BOUNDARY_DENIED', 403);
    if (input.role === 'consultant') {
      ...
      if (unsafe.rowCount) throw failure('QA_CLIENT_ISOLATION_NOT_ESTABLISHED', 403);
    }
    ...
    await client.query(`insert into qa_browser_handoffs
      ...
      values($1,$2,$3,$4,$5,$6,'PRODUCTION_QA',$7,$8,'ISSUED',$9)`,
      [id,hash(code),input.fixtureSetId,input.identityId,input.role,QA_DIET_HYDRATION_PURPOSE,input.actorId,expiresAt.toISOString(),auditReference]);
...
export const exchangeQaBrowserHandoff = async (input: { code: string; fixtureSetId: string; role: QaBrowserRole; userAgent?: string|null; ipAddress?: string|null }) => {
  const client = await pool.connect();
  try {
    await client.query('begin');
    ...
    if (String(row.fixture_set_id)!==input.fixtureSetId || row.role!==input.role || row.user_role!==input.role) throw failure('QA_HANDOFF_ROLE_OR_FIXTURE_MISMATCH',403);
    if (row.purpose!==QA_DIET_HYDRATION_PURPOSE || row.environment!=='PRODUCTION_QA' || row.account_purpose!=='QA_TEST' || row.user_status!=='active' || row.fixture_status!=='ACTIVE' || new Date(row.fixture_expires)<=new Date()) throw failure('QA_HANDOFF_BOUNDARY_DENIED',403);
    ...
    if (!consumed.rowCount) throw failure('QA_HANDOFF_ALREADY_CONSUMED');
...
export const revokeQaFixtureSessions = async (fixtureSetId: string) => {
  ...
  for (const row of result.rows) await pool.query(`insert into qa_provisioning_audit_events(id,actor_user_id,target_user_id,action,account_purpose,role,reason,metadata)
