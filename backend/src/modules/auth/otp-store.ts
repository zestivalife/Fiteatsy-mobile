...
const memoryChallenges = new Map<string, StoredOtpChallenge>();
...
const challengeKey = (id: string) => `fiteatsy:otp:challenge:${id}`;
const mobileKey = (mobile: string) => `fiteatsy:otp:mobile:${mobile}`;
const rateKey = (mobile: string) => `fiteatsy:otp:rate:${mobile}`;

const encode = (value: string) => `$${Buffer.byteLength(value)}\r\n${value}\r\n`;
const command = (parts: string[]) => `*${parts.length}\r\n${parts.map(encode).join('')}`;

const readRedisReply = (buffer: Buffer, offset = 0): { value: unknown; offset: number } => {
  const end = buffer.indexOf('\r\n', offset);
  if (end < 0) throw new Error('Incomplete Redis response.');
...
export const redisCommand = async (parts: string[]) => {
  ...
  const socket = (secure ? tls.connect({ host: url.hostname, port: Number(url.port) || 6380 }) : net.connect({ host: url.hostname, port: Number(url.port) || 6379 })) as RedisSocket;
  const chunks: Buffer[] = [];
  ...
  return await new Promise<unknown>((resolve, reject) => {
    ...
    socket.on('data', (chunk) => {
      ...
      try {
        ...
        if (replies.length === requests.length) finish(undefined, replies[replies.length - 1]);
      } catch (error) {
...
export const otpStore = {
  async get(challengeId: string): Promise<StoredOtpChallenge | null> {
    if (isTestRuntime()) return memoryChallenges.get(challengeId) ?? null;
    const value = await redisCommand(['GET', challengeKey(challengeId)]);
    return value ? (JSON.parse(String(value)) as StoredOtpChallenge) : null;
  },
  async set(challenge: StoredOtpChallenge, ttlMs: number) {
    if (isTestRuntime()) {
  ...
  async delete(challengeId: string) {
    if (isTestRuntime()) memoryChallenges.delete(challengeId);
    else await redisCommand(['DEL', challengeKey(challengeId)]);
  },
  async getActiveId(mobile: string) {
    if (isTestRuntime()) {
      return Array.from(memoryChallenges.values()).find(
        (item) => item.user.mobileNumber === mobile && !item.verified && item.expiresAtMs >= Date.now()
      )?.challengeId ?? null;
    }
    const value = await redisCommand(['GET', mobileKey(mobile)]);
    return value ? String(value) : null;
  },
  async setActiveId(mobile: string, challengeId: string, ttlMs: number) {
    if (isTestRuntime()) return;
    await redisCommand(['SET', mobileKey(mobile), challengeId, 'PX', String(ttlMs)]);
  },
  async invalidateMobile(mobile: string) {
    const id = await otpStore.getActiveId(mobile);
    if (id) await otpStore.delete(id);
    if (isTestRuntime()) {
      for (const [key, value] of memoryChallenges) if (value.user.mobileNumber === mobile) memoryChallenges.delete(key);
    } else await redisCommand(['DEL', mobileKey(mobile)]);
  },
  async rateTimestamps(mobile: string) {
    if (isTestRuntime()) return memoryRateLimits.get(mobile) ?? [];
    const values = await redisCommand(['LRANGE', rateKey(mobile), '0', '-1']);
    return (Array.isArray(values) ? values : []).map(Number).filter(Number.isFinite);
  },
  async recordRateTimestamp(mobile: string, timestamp: number, ttlMs: number) {
    if (isTestRuntime()) {
