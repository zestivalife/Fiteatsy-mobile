...
const handleCommonFoodError=(res:Response,error:unknown)=>{if(error instanceof CommonFoodApiError)return res.status(error.statusCode).json({error:error.code,message:error.message});throw error;};
