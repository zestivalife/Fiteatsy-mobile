import { NUTRITION_MEAL_SEQUENCE, type NutritionMealSlot, type NutritionMealTarget, type NutritionPlanContent } from '../platform/platform.types.js';
...
type NutrientKey = 'calories' | 'proteinGrams' | 'carbsGrams' | 'fatGrams' | 'fibreGrams';
...
const band = (value: number | null, tolerance: number, digits: number): Band => value == null
  ? { min: null, max: null }
  : { min: round(value * (1 - tolerance), digits), max: round(value * (1 + tolerance), digits) };

const allocateReconciled = (daily: number | null | undefined, digits: number) => {
  if (daily == null || !Number.isFinite(daily)) return Object.fromEntries(NUTRITION_MEAL_SEQUENCE.map((key) => [key, null]));
  const factor = 10 ** digits;
  ...
  [...base]
    ...
    .forEach((item) => { if (remaining > 0) { base.find((entry) => entry.key === item.key)!.allocated += 1; remaining -= 1; } });
  return Object.fromEntries(base.map((item) => [item.key, item.allocated / factor]));
};

export const deriveVersionedMealTargets = (daily: DailyNutritionPrescription): Record<typeof NUTRITION_MEAL_SEQUENCE[number], NutritionMealTarget> => {
  ...
  return Object.fromEntries(NUTRITION_MEAL_SEQUENCE.map((key) => {
    ...
    return [key, target];
  })) as Record<typeof NUTRITION_MEAL_SEQUENCE[number], NutritionMealTarget>;
};

const inside = (value: number | null | undefined, range: Band | undefined) => range?.min == null || range.max == null
  ? true
  : value != null && Number.isFinite(value) && value >= range.min && value <= range.max;

export const optionFitsMealTarget = (option: NutritionMealSlot, target: NutritionMealTarget) =>
  inside(option.approxKcal, target.caloriesBand) && inside(option.proteinGrams, target.proteinBand) &&
...
const scaleNullable = (value: number | null | undefined, multiplier: number) => value == null ? null : round(value * multiplier, 1);

/** Selects only practical half/quarter serving increments; it never creates a new recipe identity. */
export const optimiseMealOptionPortion = (option: NutritionMealSlot, target: NutritionMealTarget): NutritionMealSlot | null => {
  const candidates = CALORIE_MACRO_ALLOCATION_CONFIG.practicalServingMultipliers.map((multiplier) => {
    const scaled: NutritionMealSlot = {
      ...option,
...
export type AllocationValidation = { valid: boolean; code: string | null; failures: string[] };

export const validateAllocatedDiet = (content: NutritionPlanContent): AllocationValidation => {
  // Legacy versions remain readable; the new contract is enforced only on versions carrying its snapshot.
  if (content.allocationSnapshot?.methodologyVersion !== CALORIE_MACRO_ALLOCATION_METHODOLOGY_VERSION) return { valid: true, code: null, failures: [] };
  const failures: string[] = [];
  for (const key of NUTRITION_MEAL_SEQUENCE) {
    const section = content.mealPlan[key];
    if (!section.target) { failures.push(`${key}: missing meal target snapshot`); continue; }
    const families = new Set<string>();
    for (const option of section.options) {
      const family = optionFamilyIdentity(option);
      if (families.has(family)) failures.push(`${key}: duplicate canonical recipe ${family}`);
      families.add(family);
      if (!optionFitsMealTarget(option, section.target)) failures.push(`${key}: ${option.id ?? option.meal} is outside its nutrition envelope`);
    }
  }
  ...
  if (calorieTarget != null) {
    ...
    if (!inside(minimum, range) || !inside(maximum, range)) failures.push(`daily calorie choice envelope ${minimum}-${maximum} is outside ${range.min}-${range.max}`);
  }
