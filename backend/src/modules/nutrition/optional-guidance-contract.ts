import type { NutritionGuidanceItem, NutritionPlanContent, OptionalNutritionGuidance } from '../platform/platform.types.js';
import type { FoodMasterRecord } from './meal-engine.js';

export type OptionalGuidanceCompatibility = {
...
const normalized = (values: string[]) => values.map(normalize).filter(Boolean);

const enabledGuidanceItems = (guidance: OptionalNutritionGuidance) => [
  ...guidance.whatCanIEatNow,
  ...Object.values(guidance.eatingOut).flat(),
  ...Object.values(guidance.cravings).flat(),
].filter((item) => item.enabled);

const dietCompatible = (requestedValue: string | null, dietaryTags: string[]) => {
  const requested = normalize(requestedValue).replaceAll('_', '-');
  if (!requested || requested.includes('non-vegetarian') || requested.includes('non vegetarian')) return true;
  const tags = normalized(dietaryTags).map((tag) => tag.replaceAll('_', '-'));
  if (requested.includes('vegan')) return tags.some((tag) => tag.includes('vegan'));
  if (requested.includes('jain')) return tags.some((tag) => tag.includes('jain'));
  if (requested.includes('egg')) return tags.some((tag) => tag.includes('egg') || tag.includes('vegetarian') || tag.includes('vegan'));
  if (requested.includes('vegetarian')) return tags.some((tag) => (!tag.includes('non-') && tag.includes('vegetarian')) || tag.includes('vegan') || tag.includes('jain'));
  return tags.some((tag) => requested.includes(tag) || tag.includes(requested));
};

const unresolved = (item: NutritionGuidanceItem) =>
  !item.foodId ||
  !item.name.trim() ||
...
export const validateOptionalGuidanceV2 = (input: {
  ...
  verifiedActiveFoods: FoodMasterRecord[];
  compatibility: OptionalGuidanceCompatibility;
  ...
  for (const item of items) {
    ...
    const food = foodsById.get(item.foodId as string);
