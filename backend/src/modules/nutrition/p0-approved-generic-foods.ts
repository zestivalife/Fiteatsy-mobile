...
const mealHeadsByCategory: Record<string, MealHead[]> = {
...
export function createP0ApprovedGenericFoods(catalogueFoods: CatalogueFood[], decisions: Array<{ sourceRecordId: string; referenceItemId: string; canonicalName: string; aliases: string[]; category: string; sourceMapping: { fdcId: number } | null; generatorEligible: boolean; componentEligible: boolean }>): CommonFood[] {
  ...
  return P0_APPROVED_GENERIC_MAPPINGS.flatMap((mapping) => {
    ...
    if (!decision || !food || !decision.sourceMapping || (!decision.generatorEligible && !decision.componentEligible)) return [];
    const n = food.nutrients;
    if (!['calories', 'proteinGrams', 'carbohydrateGrams', 'fatGrams'].every((key) => Number.isFinite(n[key as keyof typeof n]))) return [];
    const roles = decision.category.toLowerCase().includes('fruit') ? ['FRUIT'] as const : food.foodCategory === 'protein' ? ['PROTEIN'] as const : ['VEGETABLE'] as const;
    return [{
