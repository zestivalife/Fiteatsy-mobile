...
import { getConsultantConsent, getNotificationPreferences, isConsultantConsentGranted, setConsultantConsent, updateNotificationPreferences } from './preferences.repository.js';
...
preferencesRouter.get('/consultant-access',async(req,res)=>res.json({consent:await getConsultantConsent(getAuthenticatedAccount(req).user.id)}));
preferencesRouter.put('/consultant-access',async(req,res)=>{const parsed=z.object({status:z.enum(['GRANTED','REVOKED']),policyVersion:z.literal('CONSULTANT_ACCESS_V1')}).safeParse(req.body);if(!parsed.success)return res.status(400).json({error:'INVALID_CONSENT'});const a=getAuthenticatedAccount(req);return res.json({consent:await setConsultantConsent(a.user.id,a.client.id,parsed.data.status,'PROFILE',parsed.data.policyVersion)});});
export const requireGrantedConsultantAccess=async(req:any,res:any,next:any)=>{try{return await isConsultantConsentGranted(String(req.params.clientId))?next():res.status(403).json({error:'CONSULTANT_ACCESS_CONSENT_REQUIRED'});}catch(error){next(error);}};
