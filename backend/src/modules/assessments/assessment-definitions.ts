...
export const ASSESSMENT_TYPE_PSS10 = 'PSS10' as const;
export const PSS10_INSTRUMENT_VERSION = 'pss10-nuetra-v17.37' as const;
export const PSS10_SCORING_VERSION = 'pss10-scoring-v1' as const;
export const PSS10_INTERPRETATION_VERSION = 'pss10-interpretation-v1' as const;

export type Pss10InterpretationKey = 'LOW' | 'MODERATE' | 'HIGH';

export type Pss10Interpretation = {
  key: Pss10InterpretationKey;
  label: 'Low stress' | 'Moderate stress' | 'High perceived stress';
};

export type AssessmentType = typeof ASSESSMENT_TYPE_PSS10;
...
export const pss10Items: AssessmentItem[] = [
...
export const pss10ResponseOptions: AssessmentResponseOption[] = [
  { value: 0, label: 'Never' },
...
export const getPss10Interpretation = (score: number): Pss10Interpretation => {
  if (!Number.isInteger(score) || score < 0 || score > 40) {
    throw new Error('PSS-10 score is outside the expected 0-40 range.');
  }
  if (score <= 13) return { key: 'LOW', label: 'Low stress' };
  if (score <= 26) return { key: 'MODERATE', label: 'Moderate stress' };
  return { key: 'HIGH', label: 'High perceived stress' };
};

export const getAssessmentDefinition = (assessmentType: AssessmentType) => {
...
export const normalizeAssessmentType = (assessmentType: string): AssessmentType | null =>
  assessmentType.toUpperCase() === ASSESSMENT_TYPE_PSS10 ? ASSESSMENT_TYPE_PSS10 : null;
