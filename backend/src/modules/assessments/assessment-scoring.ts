...
export type AssessmentScore = {
  assessmentType: typeof ASSESSMENT_TYPE_PSS10;
  rawScore: number;
  maxScore: number;
  scoringVersion: typeof PSS10_SCORING_VERSION;
  interpretationVersion: typeof PSS10_INTERPRETATION_VERSION;
  interpretation: ReturnType<typeof getPss10Interpretation>;
  itemScores: Array<{
    itemId: string;
...
export const scorePss10 = (responses: AssessmentResponseInput[]): AssessmentScore => {
  const expectedIds = new Set(pss10Items.map((item) => item.id));
  const responseMap = new Map<string, AssessmentResponseInput>();
