...
create table if not exists food_catalogue_usda_activation_v17_33a2 (
  activation_id text primary key,
  reference_item_id text not null unique references food_catalogue_reference_items(id),
  activation_type text not null check(activation_type in ('NEW_USDA_MAPPING','ALIAS_EXISTING')),
  ...
  processor_version text not null check(processor_version = 'FOOD_USDA_ACTIVATION_V17_33A2'),
  activation_payload jsonb not null,
  created_at timestamptz not null default now(),
  check ((activation_type = 'ALIAS_EXISTING') = (alias_target_food_id is not null)),
  check (not generator_eligible or activation_type = 'NEW_USDA_MAPPING'),
  check (not direct_add_eligible or component_eligible)
);

create table if not exists food_catalogue_usda_activation_audit_v17_33a2 (
  id uuid primary key,
  activation_id text not null references food_catalogue_usda_activation_v17_33a2(activation_id),
...
create unique index if not exists food_catalogue_usda_activation_v17_33a2_new_fdc_idx
  on food_catalogue_usda_activation_v17_33a2(selected_fdc_id)
  where activation_type = 'NEW_USDA_MAPPING';

create index if not exists food_catalogue_usda_activation_v17_33a2_role_idx
  on food_catalogue_usda_activation_v17_33a2 using gin(roles);
