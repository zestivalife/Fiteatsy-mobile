...
const assert = (condition, code) => { if (!condition)
...
const metrics = (values) => ({ samples: values.length, p50Ms: percentile(values, .5), p95Ms: percentile(values, .95) });
...
const main = async () => {
    ...
    const call = async (token, method, path, body) => {
        ...
        return { body: parsed, status: response.status, ms: performance.now() - started, bytes, headers: response.headers };
    };
    const ok = async (token, method, path, body, expected = 200) => {
    ...
    const clientId = (role) => String(identities[role].fiteatsy_client_id);
    const pathFor = (role) => `/v1/consultants/clients/${clientId(role)}`;
    const report = { fixtureSetId: String(fixture.rows[0].id), fixtureCode: FIXTURE_CODE, migrations: migrations.rows.map(r => r.version), auth: {}, coverage: {}, explorer: {}, editing: {}, safety: {}, rbac: {}, concurrency: {}, lifecycle: {}, parity: {}, performance: {}, observability: {} };
    const visible = await ok(tokens.consultant, 'GET', '/v1/consultants/clients');
    report.auth.consultant = visible.body.clients.some((x) => x.clientId === clientId('vegetarian'));
    ...
    for (const role of ['vegetarian', 'egg', 'non_vegetarian', 'vegan']) {
        ...
        if (role === 'vegan') {
            ...
            const count = generated.body.meals.reduce((n, m) => n + m.options.length, 0);
            assert(generated.body.meals.length === 7 && count === 35 && generated.body.meals.every((m) => m.options.length === 5 && new Set(m.options.map((o) => o.diversitySignature)).size === 5), `COVERAGE_35_FAILED:${role}`);
    ...
    const searchable = catalogue.body.items?.find((item) => item.displayName && item.servings?.length);
    assert(searchable, 'ACTIVE_SEARCHABLE_CATALOGUE_FOOD_REQUIRED');
    const exactTerm = encodeURIComponent(searchable.displayName);
    const explorerTimes = [];
    for (let i = 0; i < 20; i++)
        explorerTimes.push((await ok(tokens.consultant, 'GET', `${vegBase}/common-foods?mealHead=BREAKFAST&search=${exactTerm}&limit=10&offset=0`)).ms);
    const exact = await ok(tokens.consultant, 'GET', `${vegBase}/common-foods?mealHead=BREAKFAST&search=${exactTerm}&limit=10&offset=0`);
    assert(exact.body.items?.some((item) => item.id === searchable.id), 'EXACT_CATALOGUE_SEARCH_FAILED');
    const governedAlias = searchable.aliases?.find((alias) => alias.trim() && alias.toLowerCase() !== searchable.displayName.toLowerCase());
    ...
    if (governedAlias) {
        ...
        assert(aliasResult?.items?.some((item) => item.id === searchable.id), 'GOVERNED_ALIAS_SEARCH_FAILED');
    }
    ...
    report.explorer = { search: 'PASS', searchTerm: searchable.displayName, foodId: searchable.id, alias: governedAlias ? 'PASS' : 'NOT_AVAILABLE', aliasTerm: governedAlias ?? null, filters: 'PASS', pagination: 'PASS', serving: exact.body.items.every((x) => x.servings.length > 0) ? 'PASS' : 'FAIL' };
    ...
    const cucumber = cucumberSearch.body.items.find((item) => item.id === 'BATCH0_42');
    const tofu = (await ok(tokens.consultant, 'GET', `${vegBase}/common-foods?scope=RECOMMENDED&mealHead=BREAKFAST&search=tofu&limit=10&offset=0`)).body.items.find((item) => item.id === 'BATCH0_218');
    const banana = (await ok(tokens.consultant, 'GET', `${vegBase}/common-foods?scope=RECOMMENDED&mealHead=BREAKFAST&search=banana&limit=10&offset=0`)).body.items.find((item) => item.id === 'BATCH0_103');
    ...
    assert(p0Added.body.components.some((component) => component.foodId === 'BATCH0_42'), 'P0_ADD_TO_MEAL_PERSISTENCE_FAILED');
    report.explorer.p0ActivatedAddToMeal = { result: 'PASS', foodId: 'BATCH0_42', displayName: 'Cucumber' };
    currentPhase = 'PERSIST_35_OPTIONS';
    const selectionPayload = { expectedPlanVersionId: versionId, options: generatedByRole.vegetarian.meals.flatMap((meal) => meal.options.map((option) => ({ optionId: option.combinationId, mealHead: meal.mealHead, components: option.components.map((x) => ({ foodId: x.foodId, servingId: x.servingId, multiplier: x.multiplier })) }))) };
    ...
    const persisted = repeatedSelection.body.options;
    ...
    const optionId = String(baseline.combinationId);
    const first = baseline.components[0];
    const mutationTimes = [];
    ...
    assert(stale.status === 409 && stale.body.error === 'STALE_PLAN_VERSION', 'STALE_WRITE_NOT_REJECTED');
    report.concurrency = { status: stale.status, code: stale.body.error };
    for (let i = mutationTimes.length; i < 20; i++)
        mutationTimes.push((await ok(tokens.consultant, 'PATCH', `${vegBase}/diet-plans/${planId}/common-food/options/${optionId}/components/${first.foodId}/serving`, { expectedPlanVersionId: versionId, servingId: first.servingId, multiplier: i % 2 ? 1 : 1.5 })).ms);
    ...
    const review = queue.body.reviews.find((x) => x.plan?.id === planId || x.dietPlan?.id === planId || x.id === planId);
    ...
    for (let i = generateTimes.length; i < 20; i++)
        generateTimes.push((await ok(tokens.consultant, 'POST', `${pathFor('egg')}/diet-plans/${plans.egg.plan.id}/common-food/generate`, { mealHeads: MEAL_HEADS })).ms);
    ...
    from common_food_generation_run_audit where generation_run_id = any($1::text[]) order by created_at desc`, [Object.values(report.coverage).flatMap((x) => x?.generationRunId ? [x.generationRunId] : [])]).catch(() => ({ rows: [] }));
    const fallbackAudits = await pool.query(`select id, candidate_count, eligible_count, jsonb_array_length(top_options) returned_count, duration_ms, generator_version, template_version, catalogue_snapshot_version catalogue_version from common_food_generation_runs where id::text like any($1::text[]) order by created_at desc`, [[...Object.values(report.coverage).flatMap((x) => x?.generationRunId ? [`${x.generationRunId}:%`] : [])]]);
