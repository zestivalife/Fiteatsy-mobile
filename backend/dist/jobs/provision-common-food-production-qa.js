...
const assertGuard = () => {
    if (process.env.NODE_ENV !== 'production')
        throw new Error('PRODUCTION_QA_REQUIRES_NODE_ENV_PRODUCTION');
    if (process.env.ALLOW_PRODUCTION_QA_FIXTURES !== 'true')
        throw new Error('ALLOW_PRODUCTION_QA_FIXTURES_REQUIRED');
    if (process.env.QA_FIXTURE_PURPOSE !== PURPOSE)
        throw new Error('QA_FIXTURE_PURPOSE_MISMATCH');
    if (!process.env.RAILWAY_ENVIRONMENT_ID && !process.env.RAILWAY_PROJECT_ID)
        throw new Error('RAILWAY_PRODUCTION_RUNTIME_REQUIRED');
};
const identities = [
...
const upsertFixtureSet = async () => {
    ...
    values ($1,$2,'PRODUCTION_QA',$3,'ACTIVE',$4,now() + interval '6 hours',$5::jsonb,$6)`, [id, FIXTURE_CODE, PURPOSE, ACTOR, JSON.stringify({ reportingClass: 'QA_TEST', reusable: false }), crypto.randomUUID()]);
    return id;
};
const activate = async () => {
    ...
    if (migrations.rowCount !== 5)
        throw new Error('REQUIRED_PRODUCTION_MIGRATIONS_NOT_APPLIED');
    ...
    for (const [fixtureRole, dietType] of [['vegetarian', 'vegetarian'], ['egg', 'eggetarian'], ['non_vegetarian', 'non_vegetarian'], ['vegan', 'vegan']]) {
...
const deactivate = async () => {
    ...
    if (!set.rowCount)
        throw new Error('QA_FIXTURE_SET_NOT_FOUND');
    ...
    for (const row of entities.rows.filter(row => row.entity_type === 'ASSIGNMENT'))
        await revokeQaAssignment({ actorUserId, assignmentId: String(row.entity_id), reason: REASON });
    for (const row of entities.rows.filter(row => row.entity_type === 'USER'))
        await deactivateQaIdentity({ actorUserId, userId: String(row.entity_id), reason: REASON });
