import { getRegisteredConsultantClientProfileContext, getConsultantWearableSummaryForClient, listConsultantReportSummariesForClient, listConsultantTimelineForClient, listValidatedBiomarkerSummaryForClient, } from '../consultants/consultants.repository.js';
...
import { createOrUpdateDietPlanDraft, createDietPlanDraftVersion, getDietPlanByCareCaseId, getCurrentDietPlanVersion, getDietPlanById, getDietPlanVersionById, getLatestPublishedDietPlanByClientId, updateDietPlanLifecycle, updateDietPlanVersionContent, updateDietPlanVersionExportPaths, listDietPlanReviewQueue, publishApprovedDietPlanVersion, } from './nutrition.store.js';
...
const COMPATIBLE_MEAL_LIBRARY_KEYS = {
...
const isConsultantRole = (account) => CONSULTANT_DIET_WORKFLOW_ROLES.includes(account.user.role?.toLowerCase() ?? '');
const professionalTypeForNutritionAccount = (account) => account.user.role?.toLowerCase() === 'practitioner' ? 'PRACTITIONER' : 'CONSULTANT';
export const canApproveOrPublishDietPlan = (account) => ['senior_consultant', 'admin', 'superuser', 'platform_owner'].includes(account.user.role?.toLowerCase() ?? '');
export const canPublishAssignedDietPlan = (account, hasActiveCanonicalAssignment) => canApproveOrPublishDietPlan(account) ||
...
const normalizeMealOptions = (options) => (options ?? [])
...
const dedupeMealOptions = (options) => {
    const seen = new Set();
...
const canonicalReviewOptionIdentity = (option) => {
    if (option.id?.trim())
        return `option:${option.id.trim().toLowerCase()}`;
    const foodIds = unique((option.components ?? []).map((component) => component.foodId)).sort();
    if (foodIds.length > 0)
        return `foods:${foodIds.join('+')}`;
 ...
 */
export const assertDietPlanReviewContentComplete = (content) => {
    ...
    for (const mealKey of NUTRITION_MEAL_SEQUENCE) {
        ...
        options.forEach((option, index) => {
            ...
            if (identities.has(identity))
                failures.push(`${mealKey}: duplicate option ${index + 1}`);
            identities.add(identity);
        });
    }
    if (mealPlan)
        failures.push(...validateAllocatedDiet(content).failures);
    if (failures.length > 0) {
        throw new NutritionPlanWorkflowError('DIET_PLAN_REVIEW_CONTENT_INCOMPLETE', `Complete the saved diet version before review: ${failures.join('; ')}.`, 409);
    }
};
export const assertCommonFoodReviewOptionsComplete = (options) => {
    const failures = [];
    for (const mealHead of COMMON_FOOD_MEAL_HEADS) {
        const mealOptions = options.filter((option) => option && typeof option === 'object'
            && option.mealHead === mealHead);
        ...
        mealOptions.forEach((raw, index) => {
            ...
            if (!identity || identities.has(identity))
                failures.push(`${mealHead}: duplicate option ${index + 1}`);
            identities.add(identity);
        });
    }
    if (failures.length > 0) {
        throw new NutritionPlanWorkflowError('DIET_PLAN_REVIEW_CONTENT_INCOMPLETE', `Complete the saved common-food diet version before review: ${failures.join('; ')}.`, 409);
    }
};
const assertDietPlanVersionReviewComplete = (version) => {
    if ((version.commonFoodOptions?.length ?? 0) > 0)
        assertCommonFoodReviewOptionsComplete(version.commonFoodOptions);
    else
        assertDietPlanReviewContentComplete(version.content);
};
export const classifyEatingOutRecommendation = (optionId, activePublishedOptionIds) => optionId && activePublishedOptionIds.has(optionId) ? 'approved' : 'general';
const normalizeMealSection = (section) => {
...
const normalizeNutritionPlanContent = (content) => ({
    ...content,
    mealPlan: {
...
const stripAvailableOptionsFromSection = (section) => {
...
export const sanitizePublishedNutritionPlanContent = (content) => ({
    ...content,
    mealPlan: {
...
const getLatestDownloadableDietPlanVersion = async (plan) => {
    if (!plan)
        return null;
...
const inferRiskLevel = (signals) => {
    ...
    if ((signals.bmi ?? 0) >= 30)
        score += 2;
    else if ((signals.bmi ?? 0) >= 25)
        score += 1;
    if (signals.biomarkers.some((item) => item.clinicalStatus === 'LOW' || item.clinicalStatus === 'HIGH'))
        score += 1;
    if (['sedentary', 'inactive', 'light'].includes(lower(signals.activityLevel)))
        score += 1;
    if (['poor', 'worst', 'fair', 'average'].includes(lower(signals.sleepQuality)))
        score += 1;
    if (score >= 4)
        return 'high';
    if (score >= 2)
...
export const buildNutritionIntelligence = (input) => {
    ...
    const nutritionFocus = new Set();
    const consultantActions = new Set();
    const sourceForBiomarker = (biomarker) => `biomarkers.${biomarker.biomarkerId}.report.${biomarker.sourceReportId ?? 'unknown'}`;
    if ((input.bmi ?? 0) >= 25) {
        observations.push({
...
const buildCanonicalMealComponent = (componentName, householdLabel, nutrition) => ({
    componentName,
    quantity: 1,
...
const buildCanonicalMealLibraryFallback = (input) => {
    ...
    return (definitions[input.mealKey] ?? []).map((definition, index) => buildCanonicalMealSlot({
...
const mealSection = (mealKey, window, focus, target) => ({
    window,
    focus,
    target,
    recommendationSets: [],
...
const enrichRecommendationSets = (content) => normalizeNutritionPlanContent(content);
const mealOptionDiversityIdentity = (option) => {
    if (option.canonicalFamilyId?.trim())
        return `recipe:${lower(option.canonicalFamilyId)}`;
    const foodIds = unique((option.components ?? []).map((component) => component.foodId)).sort();
    if (foodIds.length > 0)
        return `foods:${foodIds.join('+')}`;
    return `meal:${lower(option.meal)}::${lower(option.portion)}`;
};
const targetDistance = (option, target) => {
    const terms = [
    ...
    return terms.reduce((score, [actual, expected]) => score + (actual == null || expected == null || expected <= 0 ? 0 : Math.abs(actual - expected) / expected), 0);
};
export const optimiseDistinctMealFamilies = (candidates, target) => {
...
export const selectDiverseMealOptions = (candidates, usedIdentities, limit = AVAILABLE_LIBRARY_MATCH_LIMIT) => {
...
const enrichMealPlanWithLibraryMatches = async (input) => {
    ...
    const nextMealPlanEntries = [];
    for (const mealKey of NUTRITION_MEAL_SEQUENCE) {
        const section = input.content.mealPlan[mealKey];
        const rawVerifiedMatches = dedupeMealOptions((await Promise.all(COMPATIBLE_MEAL_LIBRARY_KEYS[mealKey].map((compatibleMealKey) => listMealLibrarySlotsForTarget({
    ...
    return {
        ...input.content,
        mealPlan: Object.fromEntries(nextMealPlanEntries),
    };
};
const foodPreferenceSearchText = (slot) => lower([
    slot.meal,
    slot.portion,
    slot.prepNote,
    ...(slot.cuisineTags ?? []),
...
const containsFoodPreferenceTerm = (searchable, term) => {
...
export const assertDietPlanRespectsFoodPreferences = (content, profile, healthProfile) => {
    ...
    for (const mealKey of NUTRITION_MEAL_SEQUENCE) {
        ...
        for (const slot of [...section.options, ...(section.availableOptions ?? [])]) {
            ...
            if (!blockedTerm && !blockedComponent && dietCompatible && dairyCompatible)
                continue;
            const conflict = blockedTerm ?? blockedComponent?.componentName ?? profile?.dietType ?? 'dairy preference';
            throw new NutritionPlanWorkflowError('DIET_PLAN_FOOD_PREFERENCE_CONFLICT', `${section.focus || mealKey} contains “${slot.meal}”, which conflicts with the client's hard food preference “${conflict}”.`, 409);
        }
    }
};
...
const transitionCareCaseStageBestEffort = async (careCase, nextStage, detail) => {
    try {
...
const contentSummaryFromContent = (content) => ({
...
export const canAccessConsultantNutritionClient = async (publicClientId, account, options = {}) => {
    if (!isConsultantRole(account))
        return false;
    const useSeniorAuthority = options.allowSeniorAuthority === true && canApproveOrPublishDietPlan(account);
    const context = await getRegisteredConsultantClientProfileContext(publicClientId, useSeniorAuthority ? undefined : account.accountId, professionalTypeForNutritionAccount(account));
    if (!context)
        return false;
...
export const assertLifecycleTransition = (currentLifecycle, nextLifecycle) => {
    ...
    if (!allowedTransitions[current]?.includes(nextLifecycle)) {
        throw new NutritionPlanWorkflowError('INVALID_DIET_PLAN_TRANSITION', `Diet plan transition from ${current} to ${nextLifecycle} is not allowed.`);
    }
};
export const assertPublishVersionEligibility = (input) => {
    ...
    if (input.requestedVersionLifecycle === 'published' &&
...
export const classifyDietPlanDeliveryLifecycle = (input) => {
    if (input.latestPublishedVersionId != null &&
    ...
    if (lifecycle === 'approved' || input.planStatus === 'approved')
        return 'APPROVED_NOT_PUBLISHED';
    if (lifecycle === 'submitted_for_review' || lifecycle === 'changes_requested')
        return 'PENDING_APPROVAL';
    return 'PREPARING';
};
const buildMacroTargets = (tdee) => {
    if (tdee == null)
        return null;
...
const getWorkspaceContext = async (publicClientId, account, options = {}) => {
    if (!isConsultantRole(account))
        return null;
    const useSeniorAuthority = options.allowSeniorAuthority === true && canApproveOrPublishDietPlan(account);
    const context = await getRegisteredConsultantClientProfileContext(publicClientId, useSeniorAuthority ? undefined : account.accountId, professionalTypeForNutritionAccount(account));
    ...
    return {
        context,
        access: {
            source: useSeniorAuthority ? 'senior_consultant_authority' : 'cap003_professional_assignment',
...
export const getConsultantNutritionIntelligence = async (publicClientId, account) => {
    ...
    const stressAssessment = (workspace.latestStressAssessment?.payload?.result ?? null);
...
export const generateConsultantDietPlanDraft = async (publicClientId, account, input) => {
    if (!isConsultantRole(account))
        return null;
    const workspace = await getWorkspaceContext(publicClientId, account);
    if (!workspace || !workspace.careCase)
        return null;
    ...
    await transitionCareCaseStageBestEffort(careCase, 'ai_draft_generated', 'Nutrition draft generated for consultant review.');
    return {
        plan: saved.plan,
...
export const updateConsultantDietPlanDraft = async (publicClientId, account, dietPlanId, input) => {
    if (!isConsultantRole(account))
        return null;
    const workspace = await getWorkspaceContext(publicClientId, account);
    if (!workspace)
    ...
    if (!plan || plan.careCaseId !== workspace.careCase?.id)
        return null;
    const currentVersion = plan.currentVersionId ? await getCurrentDietPlanVersion(plan.id) : null;
    if (!currentVersion)
        return null;
    if (!['draft', 'changes_requested'].includes(currentVersion.lifecycleStatus)) {
        throw new NutritionPlanWorkflowError('DIET_PLAN_NOT_EDITABLE', 'Only draft plans or plans returned for changes can be edited.');
    }
    const foodPreferences = await getFoodPreferenceProfile(publicClientId);
    if (!input.allowIncompleteMealPlan)
    ...
    const sourceSnapshot = buildSourceSnapshot({
        ...
        stressAssessment: (workspace.latestStressAssessment?.payload?.result ?? null),
        foodPreferences: foodPreferences ? { profile: foodPreferences.profile, updatedAtISO: foodPreferences.updatedAtISO } : undefined,
    });
    const draftGuidance = input.content.optionalGuidance ? {
        ...
        eatingOut: Object.fromEntries(Object.entries(input.content.optionalGuidance.eatingOut).map(([key, items]) => [key, items.map((item) => ({ ...item, clinicallyReviewed: false }))])),
        cravings: Object.fromEntries(Object.entries(input.content.optionalGuidance.cravings).map(([key, items]) => [key, items.map((item) => ({ ...item, clinicallyReviewed: false }))])),
    } : undefined;
...
const guidanceNutritionComplete = (slot) => [slot.approxKcal, slot.proteinGrams, slot.carbsGrams, slot.fatGrams, slot.fibreGrams]
...
const guidanceItemFromSlot = (input) => {
    ...
    return {
        ...
        nutrition: {
            calories: input.slot.approxKcal,
            protein: input.slot.proteinGrams,
            carbs: input.slot.carbsGrams,
            fat: input.slot.fatGrams,
            fibre: input.slot.fibreGrams,
        },
...
const assertOptionalGuidanceValid = async (publicClientId, content, requireReviewed = false) => {
...
export const generateConsultantOptionalGuidance = async (publicClientId, account, dietPlanId) => {
    if (!isConsultantRole(account) || account.user.role?.toLowerCase() === 'senior_consultant')
        return null;
    const workspace = await getWorkspaceContext(publicClientId, account);
    if (!workspace?.careCase)
        return null;
    const plan = await getDietPlanById(dietPlanId);
    if (!plan || plan.careCaseId !== workspace.careCase.id)
        return null;
    ...
    const withinCalorieBand = (slot, mealTarget) => {
        if (slot.approxKcal == null || mealTarget.calories == null)
            return false;
    ...
    const mealTagsForSlot = (slot) => planOptions.filter((entry) => entry.slot.id === slot.id).map((entry) => entry.mealKey);
    ...
    const uniqueSlots = (slots) => Array.from(new Map(slots.map((slot) => [slot.id ?? slot.meal, slot])).values());
    ...
    const usedCuisineIds = new Set();
    const eatingOutEntries = [];
    ...
    const eatingOut = Object.fromEntries(eatingOutEntries);
    ...
    const mergeGuidanceItems = (existing, generated, maximum) => Array.from(new Map([...generated, ...(existing ?? [])].map((item) => [item.id || `${item.name}:${item.servingLabel}`, item])).values())
        .slice(0, maximum)
        .map((item, index) => ({ ...item, displayOrder: index + 1 }));
    const existingGuidance = version.content.optionalGuidance;
    const optionalGuidance = existingGuidance ? {
        ...generatedGuidance,
        ...
        eatingOut: Object.fromEntries(Object.keys(cuisineDefinitions).map((key) => [
            key,
            mergeGuidanceItems(existingGuidance.eatingOut[key], generatedGuidance.eatingOut[key], OPTIONAL_GUIDANCE_CUISINE_DISPLAY_LIMIT),
        ])),
        cravings: Object.fromEntries(Object.keys(cravingDefinitions).map((key) => [
            key,
            mergeGuidanceItems(existingGuidance.cravings[key], generatedGuidance.cravings[key], OPTIONAL_GUIDANCE_CRAVING_DISPLAY_LIMIT),
        ])),
    } : generatedGuidance;
...
export const searchConsultantOptionalGuidanceCandidates = async (publicClientId, account, dietPlanId, input) => {
    if (!isConsultantRole(account))
        return null;
    const workspace = await getWorkspaceContext(publicClientId, account, { allowSeniorAuthority: true });
    if (!workspace?.careCase)
    ...
    if (!plan || !version || plan.careCaseId !== workspace.careCase.id)
        return null;
    const preferences = await resolveFoodPreferencesFilter(publicClientId);
...
export const submitConsultantDietPlanForReview = async (publicClientId, account, dietPlanId) => {
    if (!isConsultantRole(account) || account.user.role?.toLowerCase() === 'senior_consultant')
        return null;
    const workspace = await getWorkspaceContext(publicClientId, account);
    if (!workspace?.careCase)
        return null;
    const plan = await getDietPlanById(dietPlanId);
    if (!plan || plan.careCaseId !== workspace.careCase.id || !plan.currentVersionId)
        return null;
    const version = await getCurrentDietPlanVersion(plan.id);
    if (!version)
        return null;
    const frozenCommonFood = await freezeCombinationOptionsForLifecycle(plan.id, version.id);
    if (frozenCommonFood.options.length > 0)
        assertCommonFoodReviewOptionsComplete(frozenCommonFood.options);
    else
        assertDietPlanReviewContentComplete(version.content);
...
export const requestConsultantDietPlanChanges = async (publicClientId, account, dietPlanId, comment) => {
...
export const getSeniorConsultantDietPlanReviewQueue = async (account) => {
    if (!canApproveOrPublishDietPlan(account)) {
    ...
    return reviews.map((review) => {
        try {
            ...
            return { ...review, contentValidation: { status: 'ready' } };
        }
        catch (error) {
...
export const approveConsultantDietPlan = async (publicClientId, account, dietPlanId) => {
    if (!isConsultantRole(account))
        return null;
    if (!canApproveOrPublishDietPlan(account)) {
        throw new NutritionPlanWorkflowError('ROLE_NOT_ALLOWED', 'Only Senior Consultants can approve nutrition plans.', 403);
    }
    const workspace = await getWorkspaceContext(publicClientId, account, { allowSeniorAuthority: true });
    if (!workspace)
        return null;
    ...
    const reviewedGuidance = guidance ? {
        ...guidance,
        ...
        eatingOut: Object.fromEntries(Object.entries(guidance.eatingOut).map(([key, items]) => [key, items.map((item) => ({ ...item, clinicallyReviewed: true }))])),
        cravings: Object.fromEntries(Object.entries(guidance.cravings).map(([key, items]) => [key, items.map((item) => ({ ...item, clinicallyReviewed: true }))])),
    } : undefined;
...
export const publishConsultantDietPlan = async (publicClientId, account, dietPlanId, approvedVersionId) => {
    if (!isConsultantRole(account))
        return null;
    const workspace = await getWorkspaceContext(publicClientId, account, { allowSeniorAuthority: true });
    if (!workspace || !workspace.careCase)
    ...
    if (!canPublishAssignedDietPlan(account, workspace.access.source === 'cap003_professional_assignment')) {
        throw new NutritionPlanWorkflowError('ROLE_NOT_ALLOWED', 'Only the assigned Consultant or an authorised Senior Consultant can publish this nutrition plan.', 403);
    }
    const plan = await getDietPlanById(dietPlanId);
    if (!plan || plan.careCaseId !== workspace.careCase.id)
        return null;
    ...
    await transitionCareCaseStageBestEffort(workspace.careCase, 'diet_published', 'Consultant published the nutrition plan.');
    return result;
};
export const getConsultantLatestDietPlan = async (publicClientId, account) => {
    const workspace = await getWorkspaceContext(publicClientId, account, { allowSeniorAuthority: true });
    if (!workspace || !workspace.careCase)
        return null;
...
export const exportConsultantDietPlanDocument = async (publicClientId, account, dietPlanId) => {
    if (!isConsultantRole(account))
        return null;
    const workspace = await getWorkspaceContext(publicClientId, account, { allowSeniorAuthority: true });
    if (!workspace || !workspace.careCase)
...
export const logNutritionMealConsumption = async (owner, input) => {
    const published = await getLatestPublishedDietPlanByClientId(owner);
    if (!published) {
        throw new NutritionPlanWorkflowError('DIET_PLAN_NOT_FOUND', 'A published nutrition plan is required before meal completion can be tracked.', 404);
    }
    if (published.plan.id != input.planId || published.version.id != input.versionId) {
        throw new NutritionPlanWorkflowError('DIET_PLAN_VERSION_MISMATCH', 'Meal completion can only be logged against the latest published nutrition plan.', 409);
    }
...
export const getPublishedDietPlanForClient = async (owner) => {
    ...
    return {
        ...
        version: {
            ...
            commonFoodOptions: payload.version.commonFoodOptions.map((option) => {
                const item = option;
                const components = Array.isArray(item.components) ? item.components.map((component) => { const c = component; return { id: c.componentId, name: c.foodDisplayNameSnapshot, serving: c.servingDisplayNameSnapshot, multiplier: c.multiplier, grams: c.grams, millilitres: c.millilitres, nutrition: c.nutrition }; }) : [];
...
export const getDietPlanDeliveryStatusForClient = async (owner) => {
    const careCase = await getCareCaseByClientId(owner.clientId);
    if (!careCase)
        return { status: 'NO_PLAN', plan: null };
...
export const nutritionDateKey = (value) => {
    const date = value instanceof Date ? value : new Date(value);
    const parts = nutritionDateFormatter.formatToParts(date);
    const part = (type) => parts.find((item) => item.type === type)?.value;
    return `${part('year')}-${part('month')}-${part('day')}`;
};
export const isCanonicalNutritionDate = (value) => {
    if (!canonicalNutritionDay.test(value))
        return false;
    const parsed = new Date(`${value}T00:00:00.000Z`);
    return !Number.isNaN(parsed.getTime()) && nutritionDateKey(parsed) === value;
};
export const resolveDailyNutritionTargets = (content) => {
    const options = NUTRITION_MEAL_SEQUENCE.map((key) => content.mealPlan[key].options[0]).filter(Boolean);
    const sum = (field) => {
        const values = options.map(option => option?.[field]).filter((value) => typeof value === 'number' && Number.isFinite(value));
...
export const isFutureNutritionDate = (selectedDate, now = new Date()) => {
    return selectedDate > nutritionDateKey(now);
};
export const assertCurrentNutritionBusinessDate = (eventTimeISO, now = new Date()) => {
    if (nutritionDateKey(eventTimeISO) !== nutritionDateKey(now)) {
        throw new NutritionPlanWorkflowError('NUTRITION_DATE_NOT_CURRENT', 'Nutrition entries can only be logged for the current day.', 400);
    }
};
const resolveCravingKeywords = (craving) => {
    const key = lower(craving).trim();
    if (!key)
        return [];
    if (['sweet', 'savoury', 'sugar', 'sweetness'].includes(key))
        return ['sweet', 'dessert', 'kheer', 'halwa', 'cake'];
    if (key === 'salty')
        return ['salty', 'namkeen', 'roasted', 'chat', 'papad', 'bhujia', 'chips'];
    if (key === 'crunchy')
        return ['crunchy', 'crunch', 'roasted', 'nuts', 'seed', 'munch'];
    if (key === 'spicy')
        return ['spicy', 'spice', 'masala', 'chili', 'curry', 'chhonk', 'tadka'];
    return [key];
};
const resolveCuisineLabel = (cuisine) => {
...
const eventNutritionDate = (event) => {
...
const parseNutritionEvent = (event) => {
    if (event.type !== 'meal_logged' && event.type !== 'water_logged')
        return null;
    return event.payload;
};
const scoreApprovedOption = (option, remaining) => {
    let score = 0;
    const reasons = [];
    if (remaining.protein != null && remaining.protein > 0 && (option.proteinGrams ?? 0) >= 15) {
        score += 3;
        reasons.push('HIGHER_PROTEIN');
    }
    if (remaining.fibre != null && remaining.fibre > 0 && (option.fibreGrams ?? 0) >= 5) {
        score += 2;
        reasons.push('HIGHER_FIBRE');
    }
    if (remaining.fat != null && remaining.fat <= 12 && (option.fatGrams ?? 0) <= 15) {
        score += 2;
        reasons.push('LOWER_FAT');
    }
    if (remaining.calories == null || option.approxKcal == null || option.approxKcal <= remaining.calories) {
        score += 2;
        reasons.push('CALORIE_FIT');
    }
    if (reasons.length === 0)
        reasons.push('BALANCED_OPTION');
    return { score, reasons };
};
const buildNutritionProjection = async (owner, rangeDays = 1, selectedDateISO) => {
    const published = await getLatestPublishedDietPlanByClientId(owner);
    const careCase = await getCareCaseByClientId(owner.clientId);
    if (!published || !careCase)
        return null;
    ...
    if (isFutureNutritionDate(selectedDate))
    ...
    const latestByMeal = new Map();
    for (const event of mealEvents.sort((a, b) => a.eventTimeISO.localeCompare(b.eventTimeISO))) {
        const payload = parseNutritionEvent(event);
        if (!payload || typeof payload.mealKey !== 'string')
            continue;
        latestByMeal.set(payload.mealKey, { state: payload.state ?? 'CONSUMED_APPROVED', payload, eventTimeISO: event.eventTimeISO, eventId: event.id });
    }
    ...
    for (const item of latestByMeal.values()) {
        if (item.state !== 'CONSUMED_APPROVED' && item.state !== 'CONSUMED_OUT_OF_PLAN')
            continue;
    ...
    const latestEventTimestamp = relevant.reduce((latest, event) => (latest == null || event.eventTimeISO > latest ? event.eventTimeISO : latest), null);
...
export const getClientNutritionExperience = (owner, selectedDateISO) => buildNutritionProjection(owner, 1, selectedDateISO);
export const getClientNutritionPattern = async (owner, endDateISO) => {
    const published = await getLatestPublishedDietPlanByClientId(owner);
    if (!published)
        return null;
    const careCase = await getCareCaseByClientId(owner.clientId);
    ...
    for (const event of events) {
        ...
        if (!payload || !daySet.has(day) || payload.planId !== published.plan.id || payload.versionId !== published.version.id)
            continue;
        if (event.type === 'water_logged')
            waterByDay.set(day, (waterByDay.get(day) ?? 0) + Number(payload.waterMl ?? Number(payload.litres ?? 0) * 1000));
        if (event.type === 'meal_logged' && typeof payload.mealKey === 'string')
            latest.set(`${day}:${payload.mealKey}`, { state: payload.state ?? 'CONSUMED_APPROVED', payload });
    }
    const targets = resolveDailyNutritionTargets(published.version.content);
...
export const logClientNutritionEvent = async (owner, input) => {
    const published = await getLatestPublishedDietPlanByClientId(owner);
    ...
    const section = input.mealKey === 'water'
        ? null
        : published.version.content.mealPlan[input.mealKey];
    ...
    const selectedOption = section?.options.find((option) => (input.optionId && option.id === input.optionId) ||
        (!input.optionId && input.mealName && option.meal === input.mealName));
    const selectedCombination = published.version.commonFoodOptions.find((option) => option.combinationId === input.optionId);
    ...
    await addHealthEvent({
        ...
        payload: {
            ...
            calories: selectedCombination ? selectedCombination.nutrition?.kcal ?? null : selectedOption?.approxKcal ?? input.calories ?? null,
            proteinGrams: selectedCombination ? selectedCombination.nutrition?.protein ?? null : selectedOption?.proteinGrams ?? input.proteinGrams ?? null,
            carbsGrams: selectedCombination ? selectedCombination.nutrition?.carbohydrate ?? null : selectedOption?.carbsGrams ?? input.carbsGrams ?? null,
            fatGrams: selectedCombination ? selectedCombination.nutrition?.fat ?? null : selectedOption?.fatGrams ?? input.fatGrams ?? null,
            fibreGrams: selectedCombination ? selectedCombination.nutrition?.fibre ?? null : selectedOption?.fibreGrams ?? input.fibreGrams ?? null,
...
export const logClientNutritionWater = async (owner, input) => {
    const published = await getLatestPublishedDietPlanByClientId(owner);
    if (!published || published.plan.id !== input.planId || published.version.id !== input.versionId) {
        throw new NutritionPlanWorkflowError('DIET_PLAN_VERSION_MISMATCH', 'This nutrition plan is no longer current.', 409);
    }
...
const mealWindowMinutes = (window) => {
...
const buildRecipeContext = (experience) => {
    ...
    const section = experience.version.content.mealPlan[meal.key];
    return {
...
export const scoreNutritionRecommendation = (option, remaining) => {
...
const reviewedGuidanceToRecommendation = (item) => ({
    id: item.id,
    mealName: item.name,
...
const guidanceItemToMealSlot = (item) => ({
    id: item.id,
    slot: item.displayOrder,
    meal: item.name,
    portion: item.servingLabel,
...
const rankReviewedGuidance = (items, remaining) => sortRecommendations(items
...
const mapTextList = (value) => (value ?? []).map((item) => lower(item));
const filterByTextMatch = (option, keywords) => {
...
const resolveFoodPreferencesFilter = async (clientId) => {
...
const recommendationReason = (item, remaining) => {
    if ((remaining.protein ?? 0) >= 20 && (item.proteinGrams ?? 0) >= 20)
        return 'High protein fit';
    if ((remaining.fibre ?? 0) >= 5 && (item.fibreGrams ?? 0) >= 5)
        return 'Helps close fibre gap';
    if (remaining.fat != null && remaining.fat <= 10 && (item.fatGrams ?? Number.POSITIVE_INFINITY) <= remaining.fat)
        return 'Lower-fat fit';
    if (remaining.calories != null && remaining.calories <= 400 && (item.approxKcal ?? Number.POSITIVE_INFINITY) <= remaining.calories)
        return 'Light option for this time';
    return 'Balanced against today’s remaining targets';
};
const sortRecommendations = (items, remaining) => items
...
const buildRecommendationResponse = (context, recommendations, guidanceStatus = 'available') => ({
    recommendations,
    guidanceStatus,
...
export const getNutritionWhatCanIEatNow = async (owner, selectedDateISO, mealKey) => {
    const experience = await getClientNutritionExperience(owner, selectedDateISO);
    if (!experience)
        throw new NutritionPlanWorkflowError('DIET_PLAN_NOT_FOUND', 'A published nutrition plan is required.', 404);
    const context = buildRecipeContext(experience);
    const sectionKey = mealKey && nutritionMealOrder.includes(mealKey)
        ? mealKey
        : context.meal.key;
    const guidance = experience.version.content.optionalGuidance;
    if (!guidance)
        return buildRecommendationResponse(context, [], 'preparing');
...
export const getNutritionEatingOutSuggestions = async (owner, input) => {
    ...
    if (!guidance)
        return buildRecommendationResponse(context, [], 'preparing');
    const cuisineKey = { 'north indian': 'northIndian', 'south indian': 'southIndian', chinese: 'chinese', continental: 'continental', 'fast food': 'fastFood' }[requestedCuisine];
...
export const getNutritionCravingSuggestions = async (owner, input) => {
    const experience = await getClientNutritionExperience(owner, input.selectedDate);
    if (!experience)
        throw new NutritionPlanWorkflowError('DIET_PLAN_NOT_FOUND', 'A published nutrition plan is required.', 404);
    const context = buildRecipeContext(experience);
    ...
    if (!guidance)
        return buildRecommendationResponse(context, [], 'preparing');
    const cravingKey = lower(input.craving);
