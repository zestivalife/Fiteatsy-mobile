...
const ensureString = (value, fallback = 'Not available') => {
    if (value == null)
        return fallback;
...
const take = (values, count, fallback = 'Not available') => {
...
const flattenMealOptions = (section) => section.options.map((option) => {
    return {
        meal: option.meal,
...
const buildTemplateMap = (plan, version) => {
    ...
    const tokens = {
        ...
        regional_cuisine: ensureString(version.sourceSnapshot.healthProfile?.regionalCuisine),
        allergies_and_exclusions: snapshot.allergies.length ? snapshot.allergies.join(', ') : 'None recorded',
        occupation: ensureString(version.sourceSnapshot.healthProfile?.occupation),
        work_pattern: ensureString(version.sourceSnapshot.healthProfile?.workMode),
        usual_wake_sleep_time: ensureString(version.sourceSnapshot.healthProfile?.wakeTime && version.sourceSnapshot.healthProfile?.sleepTime ? `${version.sourceSnapshot.healthProfile.wakeTime} / ${version.sourceSnapshot.healthProfile.sleepTime}` : snapshot.lifestyleSummary),
    ...
    for (const raw of version.commonFoodOptions) {
        const option = raw;
        const mealHead = String(option.mealHead ?? '').toLowerCase();
        const prefix = mealHead === 'mid_morning' ? 'mid_morning' : mealHead === 'evening_snack' ? 'evening_snack' : mealHead === 'early_morning' ? 'early_morning' : mealHead === 'bedtime' ? 'bedtime' : mealHead;
        const components = Array.isArray(option.components) ? option.components : [];
        const index = version.commonFoodOptions.filter((candidate) => String(candidate.mealHead) === String(option.mealHead)).indexOf(raw) + 1;
        if (!prefix || index < 1 || index > 5)
            continue;
        tokens[`${prefix}_${index}_meal`] = components.map(c => `${ensureString(c.label, '')} ${ensureString(c.foodDisplayNameSnapshot, '')}`.trim()).join(' + ');
        tokens[`${prefix}_${index}_portion`] = components.map(c => ensureString(c.label, '')).join(' • ');
        const nutrition = option.nutrition;
        tokens[`${prefix}_${index}_kcal`] = `${nutrition?.kcal ?? 'Not available'} kcal`;
        tokens[`${prefix}_${index}_protein`] = `${nutrition?.protein ?? 'Not available'} g`;
    }
...
export const generateDietPlanDocument = async (plan, version) => {
    ...
    await Promise.all(xmlTargets.map(async (target) => {
...
export const readGeneratedDietPlanDocumentXml = async (documentPath) => {
    const zip = await JSZip.loadAsync(await fs.readFile(documentPath));
    const document = zip.file('word/document.xml');
    if (!document)
        throw new Error('Generated Diet DOCX is missing word/document.xml.');
