...
const toRecord = (value, fallback) => {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return fallback;
    return value;
};
const mapAuditFields = (row) => ({
...
const mapDietPlan = (row) => ({
    ...
    sourceSnapshot: toRecord(row.source_snapshot, {
        bmi: null,
        weightKg: null,
        biomarkers: [],
        healthProfile: {},
...
export const getDietPlanByCareCaseId = async (careCaseId) => {
...
export const getDietPlanById = async (dietPlanId) => {
    const result = await pool.query(`
      select *
      from diet_plans
      where id = $1
...
export const listDietPlanReviewQueue = async (qaFixtureSetId) => {
    const result = await pool.query(`select dp.id, dp.user_id, dp.consultant_id, dp.plan_status, dp.updated_at, dp.submitted_at,
      ...
      order by dp.submitted_at desc nulls last, dp.updated_at desc`, [qaFixtureSetId ?? null]);
    return result.rows.map((row) => ({
        ...
        version: {
            ...
            commonFoodSnapshotHash: row.common_food_snapshot_hash == null ? null : String(row.common_food_snapshot_hash),
            contentSummary: (() => {
                const summary = toRecord(row.content_summary, {});
                const content = toRecord(row.content, {});
                const dailyTargets = toRecord(content.dailyTargets, {});
                return {
                    ...summary,
...
export const getDietPlanVersionById = async (versionId) => {
    const result = await pool.query(`
      select *
...
export const getCurrentDietPlanVersion = async (dietPlanId) => {
    const result = await pool.query(`
      select dpv.*
      from diet_plans dp
      join diet_plan_versions dpv on dpv.id = dp.current_version_id
...
export const getLatestPublishedDietPlanByClientId = async (owner) => {
    const result = await pool.query(`
      select dp.*
      from care_cases cc
    ...
    const version = plan.latestPublishedVersionId
        ...
        : null;
    if (!version || version.deletedAtISO || version.status !== 'active')
        return null;
    return {
        plan,
        version,
...
export const updateDietPlanLifecycle = async (input) => {
    ...
    if (input.reviewEventType) {
        await pool.query(`insert into diet_plan_review_events (id, diet_plan_id, diet_plan_version_id, actor_user_id, event_type, comment)
       values ($1, $2, $3, $4, $5, $6)`, [crypto.randomUUID(), input.dietPlanId, input.currentVersionId, input.consultantId, input.reviewEventType, input.reviewComment ?? null]);
    }
    return {
        plan: mapDietPlan(updatedPlan.rows[0]),
...
export const publishApprovedDietPlanVersion = async (input) => {
    ...
    try {
      ...
      `, [input.dietPlanId, input.versionId, input.publishedBy, timestamp, JSON.stringify(input.sourceSnapshot)]);
        ...
        await client.query(`insert into diet_plan_review_events (id, diet_plan_id, diet_plan_version_id, actor_user_id, event_type, comment)
