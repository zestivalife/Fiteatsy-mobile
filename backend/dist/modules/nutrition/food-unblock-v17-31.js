const rolesFor = (food) => {
    if (food.foodCategory === 'vegetable')
        return ['VEGETABLE'];
    if (food.foodCategory === 'legume')
        return ['PULSE', 'PROTEIN'];
    if (food.foodCategory === 'grain')
        return ['GRAIN', 'STARCH'];
    if (food.foodCategory === 'dairy')
        return ['DAIRY', 'PROTEIN', 'BEVERAGE'];
    if (food.foodCategory === 'nuts')
        return ['NUT_SEED', 'PROTEIN'];
    if (food.foodCategory === 'seeds')
        return ['NUT_SEED'];
    if (['protein', 'egg', 'fish', 'poultry'].includes(food.foodCategory ?? ''))
        return ['PROTEIN'];
    return [];
};
const vegetarianClass = (food) => food.foodCategory === 'egg'
    ? 'EGG'
    : ['poultry', 'fish'].includes(food.foodCategory ?? '') || food.dietaryTags.includes('non-vegetarian')
        ...
        : food.dietaryTags.includes('vegan')
            ? 'VEGAN'
            : 'VEGETARIAN';
export function createFoodUnblockV1731Foods(catalogueFoods, decisions) {
    const foodsByFdcId = new Map(catalogueFoods.map((food) => [food.fdcId, food]));
    return decisions.flatMap((decision) => {
        if (decision.outcome !== 'ACTIVATED_GENERATOR' || !decision.sourceMapping || !decision.servingProfile || !decision.nutritionVector)
            return [];
        const food = foodsByFdcId.get(decision.sourceMapping.fdcId);
        if (!food)
            return [];
        const nutrients = food.nutrients;
        if (!['calories', 'proteinGrams', 'carbohydrateGrams', 'fatGrams'].every((key) => Number.isFinite(nutrients[key])))
            return [];
...
export function applyFoodUnblockV1731Aliases(foods, decisions) {
    const aliasesBySource = new Map();
    for (const decision of decisions) {
        if (decision.outcome !== 'SOURCE_MAPPED_NOT_GENERATOR' || !decision.sourceMapping)
            continue;
