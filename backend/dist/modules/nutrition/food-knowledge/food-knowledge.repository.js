...
const safeLimit = (value) => Math.min(Math.max(Math.trunc(value ?? 50), 1), 100);
export const getFoodKnowledgeProjection = async (foodId) => {
...
export const searchFoodKnowledge = async (query, limit = 30) => {
    const result = await pool.query(`select distinct p.food_id, p.canonical_code, f.display_name, p.food_type, p.client_consumable
      ...
      limit $2`, [query.trim().slice(0, 100), safeLimit(limit)]);
    return result.rows;
};
export const findEligibleFoodKnowledge = async (input) => {
    const result = await pool.query(`with recursive requested_cuisines as (
     ...
     select distinct p.*
      ...
      limit $8`, [input.mealKey ?? null, input.dietPattern ?? null, input.preparationProfiles ?? [], input.excludeComponentFoodIds ?? [], input.excludeAllergenCodes ?? [], input.cuisineCodes ?? [], input.contextCodes ?? [], safeLimit(input.limit)]);
    return result.rows;
};
export const flattenFoodKnowledgeComposition = async (foodVersionId, maximumDepth = 5) => {
    const result = await pool.query(`with recursive composition as (
     ...
     ) select distinct component_food_id, component_role, min(depth)::int as depth from composition group by component_food_id, component_role order by depth, component_food_id`, [foodVersionId, Math.min(Math.max(maximumDepth, 1), 10)]);
    return result.rows;
};
export const getFoodKnowledgeCoverage = async (releaseVersion) => {
    const result = await pool.query(`select
     ...
     where p.release_version=$1`, [releaseVersion]);
