...
].map(([code, name, unit, category], index) => ({ id: uuid(100 + index), code, name, unit, category, displayOrder: index }));
const families = [
...
const makeFood = (input) => {
    ...
    return {
        ...
        version: {
            ...
            compatibilities: [
                ...(input.diets ?? ['VEGETARIAN', 'VEGAN', 'EGGETARIAN', 'NON_VEGETARIAN']).map((code, index) => ({ id: uuid(5000 + input.id * 10 + index), dimension: 'DIET_PATTERN', code, status: 'COMPATIBLE', rationale: 'Synthetic fixture composition contract.' })),
                ...(input.profiles ?? []).map(([code, status], index) => ({ id: uuid(6000 + input.id * 10 + index), dimension: 'PREPARATION_PROFILE', code, status, rationale: 'Synthetic fixture preparation contract.' })),
            ],
            allergens: (input.allergens ?? []).map(([allergenCode, status]) => ({ allergenCode, status })),
            mealSuitability: (input.meals ?? []).map((mealKey) => ({ mealKey, suitability: 'PRIMARY' })),
            contextTags: input.contexts ?? [],
        },
    };
};
export const FOOD_KNOWLEDGE_FIXTURE_MANIFEST = {
