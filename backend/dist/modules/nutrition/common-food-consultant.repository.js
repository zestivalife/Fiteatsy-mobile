...
export async function saveCombinationOption(input) {
    ...
    try {
        ...
        if (!locked.rows[0])
            throw Object.assign(new Error('DIET_PLAN_NOT_FOUND'), { code: 'DIET_PLAN_NOT_FOUND' });
        if (String(locked.rows[0].current_version_id) !== input.expectedPlanVersionId)
      ...
      values($1,$2,$3,$4,$5,$6,'MANUAL_COMBINATION',$7,$8,$9,'NUTRITION_CATALOGUE_V1_1',$10,$11,$12,$13,$14,$15)`, [input.id, input.logicalOptionId, input.supersedesId ?? null, input.planId, input.planVersionId, input.mealHead, input.snapshot.generatorVersion, input.snapshot.rankingVersion,
...
export async function getCombinationOption(id, planId, planVersionId) {
    ...
    if (!row)
        return null;
    return { combinationId: String(row.logical_option_id), mealHead: String(row.meal_head), components: row.components_snapshot,
...
export async function listCombinationOptions(planId, planVersionId) {
...
export async function replaceCombinationOptionSelection(input) {
    const client = await pool.connect();
    try {
        await client.query('begin');
        const locked = await client.query('select current_version_id from diet_plans where id=$1 and deleted_at is null for update', [input.planId]);
        if (!locked.rows[0])
            throw Object.assign(new Error('DIET_PLAN_NOT_FOUND'), { code: 'DIET_PLAN_NOT_FOUND' });
        if (String(locked.rows[0].current_version_id) !== input.expectedPlanVersionId || input.planVersionId !== input.expectedPlanVersionId)
            throw Object.assign(new Error('STALE_PLAN_VERSION'), { code: 'STALE_PLAN_VERSION' });
        ...
        for (const option of input.options) {
            ...
            const optionHash = canonicalHash({ logicalOptionId: option.combinationId, version, option });
            await client.query(`insert into diet_plan_combination_options
    ...
    values($1,$2,$3,$4,$5,$6,'MANUAL_COMBINATION',$7,$8,$9,'NUTRITION_CATALOGUE_V1_1',$10,$11,$12,$13,$14,$15)`, [id, option.combinationId, prior.rows[0]?.id ?? null, input.planId, input.planVersionId, option.mealHead, option.generatorVersion, option.rankingVersion, option.templateVersion, JSON.stringify(option.components), JSON.stringify(option.nutrition), option.diversitySignature, JSON.stringify(option.warnings), optionHash, version]);
        ...
        const order = new Map();
        for (const option of input.options) {
            const displayOrder = (order.get(option.mealHead) ?? 0) + 1;
            order.set(option.mealHead, displayOrder);
            await client.query(`insert into diet_plan_option_selections(diet_plan_id,diet_plan_version_id,logical_option_id,option_snapshot_id,meal_head,display_order) values($1,$2,$3,$4,$5,$6)`, [input.planId, input.planVersionId, option.combinationId, snapshotIds.get(option.combinationId), option.mealHead, displayOrder]);
        }
        await client.query('commit');
    }
    catch (error) {
...
export async function freezeCombinationOptionsForLifecycle(planId, planVersionId) {
    const options = (await listCombinationOptions(planId, planVersionId)).filter((x) => x !== null);
    ...
    if (!result.rowCount)
        throw Object.assign(new Error('LIFECYCLE_SNAPSHOT_FREEZE_FAILED'), { code: 'LIFECYCLE_SNAPSHOT_FREEZE_FAILED' });
