...
export async function listReferenceCatalogueFoods(input) {
    const values = ['BATCH_0_PAN_INDIA_FOOD_SEED'];
    const where = ['batch_id=$1'];
    const bind = (value) => { values.push(value); return `$${values.length}`; };
    if (input.search) {
        const p = bind(input.search);
        where.push(`(lower(canonical_name) like '%'||lower(${p})||'%' or lower(common_names::text) like '%'||lower(${p})||'%')`);
    }
    if (input.category)
        where.push(`category=${bind(input.category)}`);
    if (input.referenceState)
        where.push(`reference_state=${bind(input.referenceState)}`);
