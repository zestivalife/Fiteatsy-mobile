...
export const APPROVED_NUTRITION_CATALOGUE_VERSION = 'FITEATSY-NUTRITION-CATALOGUE-v1.1';
export const APPROVED_NUTRITION_CATALOGUE_SHA256 = 'd59b5d8e9a62f7379a292b355b3dbd30300b3db990390d60e6a8ae9f5e30f77f';
export const APPROVED_NUTRITION_CATALOGUE_PREDECESSOR_VERSION = 'FITEATSY-NUTRITION-CATALOGUE-v1';
export const APPROVED_NUTRITION_CATALOGUE_PREDECESSOR_SHA256 = '775cf73607ea84b0da1017c6652d03c8e1a58cd03058391d555713102c6c55d5';
export const APPROVED_NUTRITION_CATALOGUE_PATH = fileURLToPath(new URL('./data/fiteatsy-nutrition-catalogue-v1.1.json', import.meta.url));
...
const assertUnique = (values, label) => {
    if (new Set(values).size !== values.length)
        throw new Error(`Catalogue validation failed: duplicate ${label}`);
};
const sameNutrients = (a, b) => {
    const keys = new Set([...Object.keys(a), ...Object.keys(b)]);
    return [...keys].every((key) => a[key] === b[key]);
};
export const validateApprovedNutritionCatalogueStructure = (input) => {
    ...
    for (const food of manifest.foods) {
        for (const key of ['calories', 'proteinGrams', 'carbohydrateGrams', 'fatGrams', 'fibreGrams']) {
            if (!(key in food.nutrients))
                throw new Error(`Catalogue validation failed: ${food.id} is missing ${key}`);
        }
        if ((food.nutrients.calories ?? 0) <= 0)
            throw new Error(`Catalogue validation failed: ${food.id} has invalid calories`);
    }
    for (const recipe of manifest.recipes) {
        if (recipe.components.some((component) => !foods.has(component.foodId)))
            throw new Error(`Catalogue validation failed: ${recipe.id} references an unknown food`);
        if (!sameNutrients(calculateRecipeNutrition(recipe.components, foods), recipe.nutritionTotals))
            throw new Error(`Catalogue validation failed: ${recipe.id} nutrition is not deterministic`);
    }
    for (const variant of manifest.mealVariants) {
        const recipe = recipes.get(variant.recipeId);
        if (!recipe)
            throw new Error(`Catalogue validation failed: ${variant.id} references an unknown recipe`);
        if (!sameNutrients(scaleNutrients(recipe.nutritionTotals, variant.portionMultiplier), variant.nutritionTotals))
            throw new Error(`Catalogue validation failed: ${variant.id} nutrition is not deterministic`);
    }
    return manifest;
};
export const validateApprovedNutritionCatalogue = (raw) => {
...
export const loadApprovedNutritionCatalogue = async () => validateApprovedNutritionCatalogue(await readFile(APPROVED_NUTRITION_CATALOGUE_PATH, 'utf8'));
...
export const loadApprovedNutritionCatalogueRelease = async (version) => {
    const release = approvedReleases[version];
    const raw = await readFile(release.path, 'utf8');
    const sha256 = createHash('sha256').update(raw).digest('hex');
    if (sha256 !== release.sha256)
        throw new Error(`Catalogue validation failed: ${version} SHA-256 is not approved (${sha256})`);
    const manifest = validateApprovedNutritionCatalogueStructure(JSON.parse(raw));
    if (manifest.catalogueVersion !== version)
        throw new Error('Catalogue validation failed: requested release does not match manifest version');
