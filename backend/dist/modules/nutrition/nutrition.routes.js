...
import { approveConsultantDietPlan, canAccessConsultantNutritionClient, generateConsultantDietPlanDraft, generateConsultantOptionalGuidance, getSeniorConsultantDietPlanReviewQueue, getConsultantLatestDietPlan, getConsultantNutritionIntelligence, getPublishedDietPlanForClient, getDietPlanDeliveryStatusForClient, getClientNutritionExperience, getClientNutritionPattern, getNutritionWhatCanIEatNow, getNutritionEatingOutSuggestions, getNutritionCravingSuggestions, logClientNutritionEvent, logClientNutritionWater, NutritionPlanWorkflowError, publishConsultantDietPlan, requestConsultantDietPlanChanges, searchConsultantOptionalGuidanceCandidates, submitConsultantDietPlanForReview, updateConsultantDietPlanDraft, exportConsultantDietPlanDocument, logNutritionMealConsumption, } from './nutrition.service.js';
...
import { autoBalanceCommonFoodOption, CommonFoodApiError, generateCommonFoodPlan, getGeneratorConcentrationReport, getGeneratorPoolReport, getNutritionVerificationQueue, mutateCommonFoodOption, persistValidatedOption, reloadCommonFoodOptions, replaceSelectedCommonFoodOptions, searchCommonFoods, validateCommonFoodOption } from './common-food-consultant.service.js';
...
const nutritionPlanContentSchema = z.object({
...
const handleCommonFoodError = (res, error) => { if (error instanceof CommonFoodApiError)
    return res.status(error.statusCode).json({ error: error.code, message: error.message }); throw error; };
...
const formatMealSectionLabel = (key) => ({
...
const buildTodayNutritionView = (content) => {
...
const handleNutritionRouteError = (res, error) => {
...
consultantNutritionRouter.get('/clients/:clientId/common-foods', async (req, res) => { const parsed = commonFoodCatalogueQuerySchema.safeParse(req.query); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT', details: parsed.error.flatten() }); try {
    return res.status(200).json(await searchCommonFoods(getAuthenticatedAccount(req), req.params.clientId, parsed.data));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.get('/common-food/reports/generator-pool', async (req, res) => { try {
    return res.json(getGeneratorPoolReport(getAuthenticatedAccount(req)));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.get('/common-food/reports/concentration', async (req, res) => { try {
    return res.json(getGeneratorConcentrationReport(getAuthenticatedAccount(req)));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.get('/common-food/verification-queue', async (req, res) => { try {
    return res.json(await getNutritionVerificationQueue(getAuthenticatedAccount(req)));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.post('/clients/:clientId/diet-plans/:dietPlanId/common-food/generate', async (req, res) => { const parsed = commonFoodGenerateSchema.safeParse(req.body ?? {}); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT', details: parsed.error.flatten() }); try {
    return res.status(200).json(await generateCommonFoodPlan(getAuthenticatedAccount(req), req.params.clientId, req.params.dietPlanId, parsed.data.mealHeads));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.post('/clients/:clientId/diet-plans/:dietPlanId/common-food/validate-option', async (req, res) => { const parsed = commonFoodOptionSchema.safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT', details: parsed.error.flatten() }); try {
    return res.status(200).json(await validateCommonFoodOption(getAuthenticatedAccount(req), req.params.clientId, req.params.dietPlanId, parsed.data));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.post('/clients/:clientId/diet-plans/:dietPlanId/common-food/options', async (req, res) => { const parsed = commonFoodPersistSchema.safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT', details: parsed.error.flatten() }); try {
    return res.status(201).json(await persistValidatedOption(getAuthenticatedAccount(req), req.params.clientId, req.params.dietPlanId, parsed.data));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.put('/clients/:clientId/diet-plans/:dietPlanId/common-food/options', async (req, res) => { const parsed = commonFoodSelectionSchema.safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT', details: parsed.error.flatten() }); try {
    return res.status(200).json(await replaceSelectedCommonFoodOptions(getAuthenticatedAccount(req), req.params.clientId, req.params.dietPlanId, parsed.data));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.get('/clients/:clientId/diet-plans/:dietPlanId/common-food/options', async (req, res) => { try {
    return res.status(200).json(await reloadCommonFoodOptions(getAuthenticatedAccount(req), req.params.clientId, req.params.dietPlanId));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.patch('/clients/:clientId/diet-plans/:dietPlanId/common-food/options/:optionId/components/:componentId', async (req, res) => { const parsed = mutationBaseSchema.extend({ component: commonFoodComponentSchema }).safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT', details: parsed.error.flatten() }); try {
    return res.status(200).json(await mutateCommonFoodOption(getAuthenticatedAccount(req), req.params.clientId, req.params.dietPlanId, req.params.optionId, parsed.data.expectedPlanVersionId, items => items.map(x => x.foodId === req.params.componentId ? parsed.data.component : x), parsed.data.draft));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.post('/clients/:clientId/diet-plans/:dietPlanId/common-food/options/:optionId/components', async (req, res) => { const parsed = mutationBaseSchema.extend({ component: commonFoodComponentSchema }).safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT', details: parsed.error.flatten() }); try {
    return res.status(200).json(await mutateCommonFoodOption(getAuthenticatedAccount(req), req.params.clientId, req.params.dietPlanId, req.params.optionId, parsed.data.expectedPlanVersionId, items => [...items, parsed.data.component], parsed.data.draft));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.delete('/clients/:clientId/diet-plans/:dietPlanId/common-food/options/:optionId/components/:componentId', async (req, res) => { const parsed = mutationBaseSchema.safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT', details: parsed.error.flatten() }); try {
    return res.status(200).json(await mutateCommonFoodOption(getAuthenticatedAccount(req), req.params.clientId, req.params.dietPlanId, req.params.optionId, parsed.data.expectedPlanVersionId, items => items.filter(x => x.foodId !== req.params.componentId)));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.patch('/clients/:clientId/diet-plans/:dietPlanId/common-food/options/:optionId/components/:componentId/serving', async (req, res) => { const parsed = mutationBaseSchema.extend({ servingId: z.string().min(1), multiplier: z.number().positive() }).safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT', details: parsed.error.flatten() }); try {
    return res.status(200).json(await mutateCommonFoodOption(getAuthenticatedAccount(req), req.params.clientId, req.params.dietPlanId, req.params.optionId, parsed.data.expectedPlanVersionId, items => items.map(x => x.foodId === req.params.componentId ? { ...x, servingId: parsed.data.servingId, multiplier: parsed.data.multiplier } : x)));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.post('/clients/:clientId/diet-plans/:dietPlanId/common-food/options/:optionId/auto-balance', async (req, res) => { const parsed = mutationBaseSchema.extend({ lockedFoodIds: z.array(z.string()).default([]) }).safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT', details: parsed.error.flatten() }); try {
    return res.status(200).json(await autoBalanceCommonFoodOption(getAuthenticatedAccount(req), req.params.clientId, req.params.dietPlanId, req.params.optionId, parsed.data));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.get('/meal-templates', async (req, res) => { const parsed = z.object({ search: z.string().trim().optional(), mealHead: mealHeadSchema.optional(), status: z.enum(['DRAFT', 'ACTIVE', 'ARCHIVED']).optional(), visibility: z.enum(['PRIVATE', 'TEAM']).optional(), limit: z.coerce.number().int().min(1).max(100).default(30), offset: z.coerce.number().int().min(0).default(0) }).safeParse(req.query); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT', details: parsed.error.flatten() }); try {
    return res.json(await listConsultantMealTemplates(getAuthenticatedAccount(req), parsed.data));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.post('/meal-templates', async (req, res) => { const parsed = templateStructureSchema.safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT', details: parsed.error.flatten() }); try {
    return res.status(201).json(await createConsultantMealTemplate(getAuthenticatedAccount(req), parsed.data));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.post('/meal-templates/:templateId/activate', async (req, res) => { try {
    return res.json(await activateConsultantMealTemplate(getAuthenticatedAccount(req), req.params.templateId));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.post('/meal-templates/:templateId/revisions', async (req, res) => { const parsed = templateStructureSchema.safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT', details: parsed.error.flatten() }); try {
    return res.status(201).json(await reviseConsultantMealTemplate(getAuthenticatedAccount(req), req.params.templateId, parsed.data));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.post('/meal-templates/:templateId/clone', async (req, res) => { const parsed = z.object({ name: z.string().trim().min(1).max(120).optional() }).safeParse(req.body ?? {}); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT' }); try {
    return res.status(201).json(await cloneConsultantMealTemplate(getAuthenticatedAccount(req), req.params.templateId, parsed.data.name));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.delete('/meal-templates/:templateId', async (req, res) => { try {
    return res.json(await archiveConsultantMealTemplate(getAuthenticatedAccount(req), req.params.templateId));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.post('/clients/:clientId/diet-plans/:dietPlanId/meal-templates/:templateId/apply', async (req, res) => { const parsed = z.object({ revisionId: z.string().uuid().optional() }).safeParse(req.body ?? {}); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT' }); try {
    return res.json(await applyConsultantMealTemplate(getAuthenticatedAccount(req), req.params.clientId, req.params.dietPlanId, req.params.templateId, parsed.data.revisionId));
}
catch (e) {
    return handleCommonFoodError(res, e);
} });
consultantNutritionRouter.get('/clients/:clientId/food-preferences', async (req, res) => {
    ...
    if (!payload)
        return res.status(404).json({ error: 'CLIENT_NOT_FOUND', message: 'Client was not found.' });
    return res.status(200).json(payload);
});
...
consultantNutritionRouter.post('/clients/:clientId/diet-plans/:dietPlanId/optional-guidance/generate', async (req, res) => {
    try {
        ...
        if (!payload)
            return res.status(404).json({ error: 'DIET_PLAN_NOT_FOUND', message: 'Unable to generate guidance for this plan.' });
        return res.status(200).json(payload);
    }
    catch (error) {
        return handleNutritionRouteError(res, error);
...
consultantNutritionRouter.get('/clients/:clientId/diet-plans/:dietPlanId/optional-guidance/candidates', async (req, res) => {
    ...
    try {
        ...
        if (!payload)
            return res.status(404).json({ error: 'DIET_PLAN_NOT_FOUND', message: 'Unable to search guidance for this plan.' });
        return res.status(200).json(payload);
    }
    catch (error) {
        return handleNutritionRouteError(res, error);
...
consultantNutritionRouter.post('/clients/:clientId/diet-plans/:dietPlanId/submit-review', async (req, res) => {
    ...
    if (!payload)
        return res.status(404).json({ error: 'DIET_PLAN_NOT_FOUND', message: 'Unable to submit the requested diet plan.' });
    return res.status(200).json(payload);
});
consultantNutritionRouter.post('/clients/:clientId/diet-plans/:dietPlanId/request-changes', async (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: 'REVIEW_COMMENT_REQUIRED', details: parsed.error.flatten() });
    let payload;
    try {
    ...
    if (!payload)
        return res.status(404).json({ error: 'DIET_PLAN_NOT_FOUND', message: 'Unable to request changes for the requested diet plan.' });
    return res.status(200).json(payload);
});
...
consultantNutritionRouter.post('/clients/:clientId/diet-plans/:dietPlanId/publish', async (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: 'APPROVED_VERSION_REQUIRED', details: parsed.error.flatten() });
    const account = getAuthenticatedAccount(req);
    let payload;
    try {
...
platformNutritionRouter.get('/food-preferences', async (req, res) => {
    ...
    if (!payload)
        return res.status(404).json({ error: 'CLIENT_NOT_FOUND', message: 'Your client profile was not found.' });
    return res.status(200).json(payload);
});
...
platformNutritionRouter.get('/nutrition-experience', async (req, res) => {
    ...
    try {
        ...
        if (!payload)
            return res.status(404).json({ error: 'DIET_PLAN_NOT_FOUND', message: 'Your nutrition plan is being prepared.' });
        return res.status(200).json(payload);
    }
    catch (error) {
        if (error instanceof NutritionPlanWorkflowError)
...
platformNutritionRouter.get('/nutrition-experience/recommendations/what-can-i-eat-now', async (req, res) => {
    ...
    const account = getAuthenticatedAccount(req);
    try {
        return res.status(200).json(await getNutritionWhatCanIEatNow({ accountId: account.accountId, clientId: account.client.id }, parsed.data.date, parsed.data.mealKey));
    }
    catch (error) {
        return handleNutritionRouteError(res, error);
    }
...
platformNutritionRouter.get('/nutrition-experience/recommendations/eating-out', async (req, res) => {
    ...
    const account = getAuthenticatedAccount(req);
    try {
        return res.status(200).json(await getNutritionEatingOutSuggestions({ accountId: account.accountId, clientId: account.client.id }, { mealKey: parsed.data.mealKey, selectedDate: parsed.data.date, cuisine: parsed.data.cuisine }));
    }
    catch (error) {
        return handleNutritionRouteError(res, error);
    }
...
platformNutritionRouter.get('/nutrition-experience/recommendations/craving', async (req, res) => {
    ...
    const account = getAuthenticatedAccount(req);
    try {
        return res.status(200).json(await getNutritionCravingSuggestions({ accountId: account.accountId, clientId: account.client.id }, { mealKey: parsed.data.mealKey, selectedDate: parsed.data.date, craving: parsed.data.craving }));
    }
    catch (error) {
        return handleNutritionRouteError(res, error);
    }
...
platformNutritionRouter.post('/nutrition-plan/consume', async (req, res) => {
    ...
    const account = getAuthenticatedAccount(req);
    try {
        const payload = await logNutritionMealConsumption({ accountId: account.accountId, clientId: account.client.id }, {
            ...parsed.data,
