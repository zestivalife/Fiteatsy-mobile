...
export const DIET_TYPES = ['vegetarian', 'eggetarian', 'non_vegetarian', 'vegan', 'jain'];
export const STAPLE_PREFERENCES = ['roti', 'rice', 'both', 'none'];
export const DAIRY_PREFERENCES = ['allowed', 'limited', 'avoid'];
const emptyProfile = () => ({
    dietType: null,
    proteins: [],
    cuisines: [],
...
const cleanList = (values) => Array.from(new Set((values ?? []).map((value) => value.trim()).filter(Boolean))).slice(0, 100);
export const normalizeFoodPreferenceProfile = (input) => ({
...
const getContext = async (publicClientId) => getRegisteredConsultantClientProfileContext(publicClientId);
export const getFoodPreferenceProfile = async (publicClientId) => {
    const context = await getContext(publicClientId);
    if (!context)
        return null;
    const result = await pool.query(`select id, food_preference_profile, food_preference_updated_by, food_preference_updated_at
       from health_profiles where client_id = $1 and deleted_at is null order by updated_at desc limit 1`, [context.internalClientId]);
    const row = result.rows[0];
    return {
        clientId: publicClientId,
        profile: normalizeFoodPreferenceProfile((row?.food_preference_profile ?? {})),
...
export const listVerifiedFoodCatalogue = async (query, limit = 30, offset = 0) => {
    const safeLimit = Math.min(Math.max(limit, 1), 50);
    const safeOffset = Math.max(offset, 0);
    const result = await pool.query(`select id, canonical_name, display_name, food_category, dietary_classification,
      ...
      limit $2 offset $3`, [query.trim(), safeLimit, safeOffset]);
    return {
...
export const updateFoodPreferenceProfile = async (publicClientId, actorUserId, actorType, input) => {
    const context = await getContext(publicClientId);
    if (!context)
        return null;
    ...
    try {
        ...
        const updated = await client.query(`update health_profiles
        ...
        returning id`, [JSON.stringify(profile), actorUserId, updatedAt, context.internalClientId]);
        if (!updated.rowCount) {
            await client.query('rollback');
            return null;
        }
        await client.query(`insert into food_preference_audit_events (id, client_id, health_profile_id, actor_user_id, actor_type, profile)
       values ($1, $2, $3, $4, $5, $6::jsonb)`, [crypto.randomUUID(), context.internalClientId, updated.rows[0].id, actorUserId, actorType, JSON.stringify(profile)]);
