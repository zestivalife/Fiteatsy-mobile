...
import { z } from 'zod';
import { buildLiveSyncPayload, connectHealthApp, getHealthApps, } from './wearables.service.js';
    ...
    try {
        ...
        const { connection, payload } = buildLiveSyncPayload({
            ...
            records: observations
                ...
                .map((item) => ({ type: item.metricType, value: item.value, recordedAtISO: item.measuredAtISO }))
        });
        return res.status(200).json({
            connection,
            payload
