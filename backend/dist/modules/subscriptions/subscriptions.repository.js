...
const toIso = (value) => (value == null ? null : new Date(String(value)).toISOString());
...
export const listActiveSubscriptionPlans = async (db = pool) => {
...
export const getActiveSubscriptionPlanById = async (planId, db = pool) => {
    const result = await db.query(`
      select
        plans.*,
        coalesce(string_agg(entitlements.entitlement_code, ',' order by entitlements.entitlement_code), '') as entitlements
...
export const listActiveEntitlementsForUser = async (userId, db = pool) => {
    const result = await db.query(`
    ...
    `, [userId]);
    return result.rows.map((row) => String(row.entitlement_code));
};
export const getCurrentActiveSubscriptionForUser = async (userId, db = pool) => {
    const result = await db.query(`
      select
        subscriptions.*,
...
export const findPaymentOrderByProviderOrderId = async (providerOrderId, db = pool) => {
    const result = await db.query(`
      select *
      from payment_orders
...
export const markPaymentOrderFailed = async (paymentOrderId) => {
    await pool.query(`
      update payment_orders
...
export const recordSubscriptionAuditEvent = async (input, db = pool) => {
    await db.query(`
...
export const markWebhookEventProcessed = async (payloadHash) => {
    await pool.query(`
...
export const markWebhookEventFailed = async (payloadHash, errorMessage) => {
    await pool.query(`
      update payment_webhook_events
...
export const listPaymentHistoryForUser = async (userId) => {
    const result = await pool.query(`
      select
