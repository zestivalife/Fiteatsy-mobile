...
const mapPlan = (row) => ({
    ...
    entitlements: Array.isArray(row.entitlements) ? row.entitlements.map((item) => String(item.code ?? '')) : [],
...
`;
export const listFoundationPlans = async (db = pool) => {
...
export const getFoundationPlan = async (planId, db = pool) => {
...
export const getLatestSubscription = async (userId, db = pool) => {
...
export const getLatestPaymentState = async (userId, db = pool) => {
    const result = await db.query(`
    select status
    from payment_orders
    where user_id = $1
...
export const listSubscriptionHistory = async (userId, db = pool) => {
    const result = await db.query(`
    select subscriptions.*, plans.code as plan_code, plans.name as plan_name
    from user_subscriptions subscriptions
    join subscription_plans plans on plans.id = subscriptions.plan_id
...
export const listEffectiveEntitlements = async (userId, db = pool) => {
    const result = await db.query(`
...
export const recordFoundationEvent = async (input, db = pool) => {
