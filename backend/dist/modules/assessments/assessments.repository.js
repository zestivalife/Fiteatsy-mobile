...
import { ASSESSMENT_TYPE_PSS10, getPss10Interpretation, PSS10_INSTRUMENT_VERSION, PSS10_INTERPRETATION_VERSION, PSS10_SCORING_VERSION } from './assessment-definitions.js';
import { scorePss10 } from './assessment-scoring.js';
const rowToSession = (row, responses = []) => ({
    id: String(row.id),
...
const listResponsesForSession = async (sessionId, client = pool) => {
...
export const createAssessmentSession = async (owner) => {
...
export const getAssessmentSession = async (owner, sessionId) => {
    const result = await pool.query(`
      select *
...
export const getLatestDraftSession = async (owner) => {
    const result = await pool.query(`
      select *
      from assessment_sessions
      where user_id = $1
...
export const saveAssessmentResponses = async (owner, sessionId, responses) => {
    ...
    try {
      ...
      `, [sessionId, owner.accountId, owner.clientId]);
        ...
        await client.query(`update assessment_sessions set updated_at = now() where id = $1`, [sessionId]);
...
export const completeAssessmentSession = async (owner, sessionId) => {
    const client = await pool.connect();
    try {
        await client.query('begin');
        const session = await client.query(`
...
export const getLatestAssessmentResult = async (owner) => {
    const result = await pool.query(`
      select *
      from assessment_results
      where user_id = $1
...
export const listAssessmentResults = async (owner, limit = 50) => {
    const result = await pool.query(`
      select *
      from assessment_results
      where user_id = $1
...
export const getAssessmentResultById = async (owner, resultId) => {
    const result = await pool.query(`
      select *
      from assessment_results
      where id = $1
