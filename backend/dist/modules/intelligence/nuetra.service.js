...
const client = env.openAiApiKey
    ...
    : null;
const toAbnormal = (parameters) => parameters.filter((p) => p.status !== 'normal');
const compact = (text) => text.replace(/\s+/g, ' ').trim();
const hasAtLeastTwoNamedValues = (text, parameters) => {
...
const fallbackSummary = (parameters, userName) => {
...
const fallbackParameterInsight = (paramName, value, unit, status, referenceRange) => {
...
export const generateNuetraSummary = async (parameters, userName) => {
...
Parameters needing attention: ${JSON.stringify(abnormalParams.map((p) => ({
        name: p.name,
        value: p.value,
        unit: p.unit,
        status: p.status,
...
export const generateParameterInsight = async (paramName, value, unit, status, referenceRange) => {
    const content = await callModel([
        {
            role: 'system',
            content: 'You are Nuetra. Write one warm, plain-language sentence explaining what an abnormal lab value means for a working professional. Max 20 words, no jargon.'
        },
        {
            role: 'user',
...
export const generateActionPlan = async (abnormalParameters) => {
...
Max 3 actions. Mention actual values. Warm tone.`
    ...
    const fallback = abnormalParameters.slice(0, 3).map((param, index) => ({
        priority: index + 1,
    ...
    return content ? safeJsonParse(content, fallback) : fallback;
};
export const generateCrossReferenceInsights = async (abnormalParams, checkInHistory) => {
...
Find meaningful connections.`
    ...
    const fallback = abnormalParams.slice(0, 2).map((param) => ({
...
export const generateNuetraChat = async (userMessage, conversationHistory, reportParameters) => {
    ...
    const messages = [
        {
            role: 'system',
            content: 'You are Nuetra, a compassionate health copilot. Be warm, specific, and plain-language. Mention exact parameter names/values when relevant. Do not diagnose. Give one practical next step.'
        },
        {
            role: 'user',
...
export const generateTrackerImprovement = async (input) => {
...
const trackerMetricFallback = (input, base) => {
    const compareText = base.compareDelta !== null
...
export const generateTrackerMetricCoaching = async (input, base) => {
    const content = await callModel([
        {
            role: 'system',
