...
export const computeRisk = (input) => {
    ...
    const burnoutFlag = burnoutRisk >= 70 ? 'alert' : burnoutRisk >= 45 ? 'watch' : 'none';
    return { stressRisk, burnoutRisk, energyDeficit, burnoutFlag };
};
export const generateOnePriority = (input) => {
...
export const generateTrackerAnalysis = (input) => {
    ...
    const trend = slope > 0.22 ? 'improving' : slope < -0.22 ? 'declining' : 'stable';
    const confidence = clamp(0.55 + Math.min(0.2, Math.abs(slope) / 3) + (input.values.length >= 7 ? 0.1 : 0), 0.5, 0.92);
