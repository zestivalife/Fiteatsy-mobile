...
const mapMedication = (row) => ({
    id: String(row.id),
    clientId: String(row.client_id),
    userId: String(row.user_id),
    name: String(row.name),
    type: String(row.medication_type),
    dosage: String(row.dosage),
    schedule: row.schedule,
    reminderSound: String(row.reminder_sound),
    status: String(row.medication_status),
    notificationEnabled: Boolean(row.notification_enabled),
    createdAtISO: toIso(row.created_at),
    updatedAtISO: toIso(row.source_updated_at ?? row.updated_at)
});
const mapMedicationLog = (row) => ({
    id: String(row.id),
    clientId: String(row.client_id),
    userId: String(row.user_id),
...
export const upsertMedicationSnapshot = async (owner, medications, logs) => {
    ...
    try {
        ...
        for (const log of logs) {
            if (!medicationIds.includes(log.medicationId))
                continue;
            await client.query(`
...
export const listMedicationsForOwner = async (owner) => {
...
export const listMedicationLogsForOwner = async (owner, fromISO, toISO) => {
    const result = await pool.query(`
      select *
