...
const normalizeUnit = (unit) => unit.trim().replace(/μ/g, 'µ').replace(/\s+/g, '').toLowerCase();
export const parseNumericReferenceRange = (referenceRange) => {
    if (!referenceRange)
        return null;
...
export const deriveBiomarkerClinicalStatus = (input) => {
    if (input.validationStatus.toLowerCase() !== 'validated' || !Number.isFinite(input.value) || !input.unit.trim())
        return 'UNKNOWN';
    const range = parseNumericReferenceRange(input.referenceRange);
    if (!range)
        return 'UNKNOWN';
    if (range.min != null && (input.value < range.min || (!range.minInclusive && input.value === range.min)))
        return 'LOW';
    if (range.max != null && (input.value > range.max || (!range.maxInclusive && input.value === range.max)))
        return 'HIGH';
    return 'NORMAL';
};
const distanceFromRange = (value, status, referenceRange) => {
    const range = parseNumericReferenceRange(referenceRange);
    if (!range)
        return null;
    if (status === 'LOW' && range.min != null)
        return range.min - value;
    if (status === 'HIGH' && range.max != null)
        return value - range.max;
    return status === 'NORMAL' ? 0 : null;
};
export const compareBiomarkerObservations = (latest, previous) => {
    if (!previous)
        return 'UNKNOWN';
    if (normalizeUnit(latest.unit) !== normalizeUnit(previous.unit))
        return 'INCOMPARABLE';
    if (latest.clinicalStatus === 'UNKNOWN' || previous.clinicalStatus === 'UNKNOWN')
        return 'UNKNOWN';
    if (latest.clinicalStatus === 'NORMAL' && previous.clinicalStatus !== 'NORMAL')
        return 'IMPROVED';
    if (latest.clinicalStatus !== 'NORMAL' && previous.clinicalStatus === 'NORMAL')
        return 'NEEDS_ATTENTION';
    if (latest.clinicalStatus === 'NORMAL' && previous.clinicalStatus === 'NORMAL')
        return 'STABLE';
    if (latest.clinicalStatus !== previous.clinicalStatus)
        return 'CHANGED';
