...
import { countBiomarkerHistory, listBiomarkerHistory, listBiomarkers } from './biomarkers.repository.js';
export const biomarkersRouter = Router();
const currentOwner = (account) => ({
    accountId: account.accountId,
    clientId: account.client.id
});
const toBiomarkerObservationDto = (observation, fiteatsyClientId) => ({
    id: observation.id,
    fiteatsyClientId,
...
biomarkersRouter.get('/history', async (req, res) => {
    ...
    const biomarkerId = typeof req.query.biomarkerId === 'string' && req.query.biomarkerId.trim() ? req.query.biomarkerId.trim() : undefined;
