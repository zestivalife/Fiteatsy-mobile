...
import { createProfessionalAssignment, discoverClientsForAssignment, discoverProfessionalsForAssignment, listClientAllocationPool, listProfessionalAssignments, revokeProfessionalAssignment } from './professional-assignments.repository.js';
export const professionalAssignmentsRouter = Router();
professionalAssignmentsRouter.use(requireAuthenticatedAccount);
export const canManageProfessionalAssignments = (role) => ['admin', 'super_admin', 'platform_owner', 'senior_consultant', 'care_operations'].includes(String(role).toLowerCase());
...
professionalAssignmentsRouter.get('/clients/search', async (req, res) => {
    if (!canManageProfessionalAssignments(getAuthenticatedAccount(req).user.role))
        return res.status(403).json({ error: 'ASSIGNMENT_PERMISSION_REQUIRED' });
...
professionalAssignmentsRouter.get('/clients/pool', async (req, res) => {
    ...
    if (String(account.user.role).toLowerCase() !== 'senior_consultant' && !['admin', 'super_admin', 'platform_owner'].includes(String(account.user.role).toLowerCase()))
        return res.status(403).json({ error: 'ASSIGNMENT_PERMISSION_REQUIRED' });
    await ensureRegisteredClientsForEligibleUsers();
    const query = typeof req.query.q === 'string' ? req.query.q : '';
    const assignmentFilter = typeof req.query.assignment === 'string' && ['all', 'unassigned', 'assigned', 'mine'].includes(req.query.assignment) ? req.query.assignment : 'all';
...
professionalAssignmentsRouter.get('/professionals', async (req, res) => {
    if (!canManageProfessionalAssignments(getAuthenticatedAccount(req).user.role))
        return res.status(403).json({ error: 'ASSIGNMENT_PERMISSION_REQUIRED' });
    const type = typeof req.query.type === 'string' && ['CONSULTANT', 'PRACTITIONER', 'MENTOR'].includes(req.query.type) ? req.query.type : undefined;
...
professionalAssignmentsRouter.get('/', async (req, res) => {
    const account = getAuthenticatedAccount(req);
    if (!canManageProfessionalAssignments(account.user.role))
        return res.status(200).json({ assignments: await listProfessionalAssignments(account.accountId) });
...
professionalAssignmentsRouter.post('/', async (req, res) => {
    ...
    const assignment = await createProfessionalAssignment({ ...parsed.data, actorUserId: account.accountId, professionalType: parsed.data.professionalType });
    if (!assignment)
