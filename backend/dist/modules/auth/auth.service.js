...
const buildOtpHash = (challengeId, otp) => crypto.createHash('sha256').update(`${challengeId}:${otp}`).digest('hex');
const buildOtp = () => {
...
export const resendOtpChallenge = (challengeId) => {
    pruneOldChallenges();
...
export const verifyOtpChallenge = (challengeId, otp) => {
    pruneOldChallenges();
    const challenge = challengeStore.get(challengeId);
    if (!challenge)
        throw asDomainError({ code: 'OTP_NOT_FOUND', message: 'Challenge not found.' });
