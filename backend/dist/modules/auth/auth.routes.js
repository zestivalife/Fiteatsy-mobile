...
import { assertQaHandoffExchangeRate, exchangeQaAdminSessionHandoff, QA_ADMIN_HANDOFF_PURPOSE } from './qa-session-handoff.js';
...
authRouter.post('/qa-session-handoff/exchange', async (req, res) => {
    ...
    try {
        ...
        if (typed.retryAfterSec)
            res.setHeader('Retry-After', String(typed.retryAfterSec));
