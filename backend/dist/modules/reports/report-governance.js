...
export const canonicalBiomarkerName = (rawName) => {
    ...
    const match = registry.find((item) => normalizeName(item.canonicalName) === normalized || item.aliases.some((alias) => normalizeName(alias) === normalized));
    return match?.canonicalName ?? rawName.trim().replace(/\s+/g, ' ');
};
export const biomarkerTier = (canonicalName) => TIER_1_BIOMARKERS.some((name) => normalizeName(name) === normalizeName(canonicalName))
    ? 1
    : [...coreAliases, ...supportingAliases].some((item) => normalizeName(item.canonicalName) === normalizeName(canonicalName))
        ? 2
        : 3;
export const biomarkerDimension = (canonicalName, fallback) => [...coreAliases, ...supportingAliases].find((item) => normalizeName(item.canonicalName) === normalizeName(canonicalName))?.dimension ??
    fallback;
export const documentHash = (buffer) => crypto.createHash('sha256').update(buffer).digest('hex');
const hasNumericReferenceRange = (range) => /(?:<|>|>=|<=)\s*-?\d+(?:\.\d+)?/.test(range) ||
...
const detectSections = (text, parameters) => {
...
const resolveRequiredTier1Biomarkers = (document, sections, parameters) => {
...
const estimateExpectedRange = (document, sections, parameters) => {
    if (!document.supported)
        return { min: 0, max: 0, basis: 'unsupported_document' };
...
const plausibleValueCheck = (parameter) => {
    ...
    if (!rule)
        return { ok: Number.isFinite(parameter.value) && Math.abs(parameter.value) < 100000, reason: '' };
...
export const buildExtractionGovernance = (text, parameters, document) => {
    ...
    const validatedCore = new Set();
    const tier2Biomarkers = new Set();
    const tier3Biomarkers = new Set();
    const failedBiomarkers = [];
    const rejectedBiomarkers = [];
    const criticalValidationFailures = [];
    const conflicts = [];
    ...
    for (const parameter of parameters) {
        const canonicalName = canonicalBiomarkerName(parameter.name);
        const tier = biomarkerTier(canonicalName);
        ...
        const validationStatus = clinicallyPlausible
            ? hasUnit && hasRange
                ? 'VALID'
                : 'NEEDS_REVIEW'
            : 'INVALID';
        ...
        if (tier === 1) {
            ...
            if (validationStatus === 'VALID')
                validatedCore.add(canonicalName);
            if (validationStatus === 'INVALID')
                criticalValidationFailures.push(plausibility.reason || `${canonicalName} failed plausibility validation.`);
        }
        else if (tier === 2) {
        ...
        if (typeof previousValue === 'number' && Math.abs(previousValue - parameter.value) > Math.max(0.2, Math.abs(previousValue) * 0.15)) {
            ...
            if (tier === 1)
                criticalValidationFailures.push(conflict);
        }
        if (typeof previousValue !== 'number')
            seenValues.set(canonicalName, parameter.value);
    }
    ...
    const confidenceByRequiredBiomarker = new Map();
    for (const parameter of parameters) {
        const canonicalName = canonicalBiomarkerName(parameter.name);
        if (!presentRequiredTier1.includes(canonicalName))
            continue;
        confidenceByRequiredBiomarker.set(canonicalName, Math.max(confidenceByRequiredBiomarker.get(canonicalName) ?? 0, parameter.extractionConfidence ?? 0.75));
    }
    const requiredTier1Confidence = confidenceByRequiredBiomarker.size === 0
        ? 0
    ...
    const confidence = Number(((requiredTier1Confidence * 0.45) + (tier1ValidationConfidence * 0.35) + (requiredTier1Coverage * 0.2)).toFixed(2));
    ...
    if (parameters.length < 3)
        reasons.push(`Only ${parameters.length} biomarkers detected; minimum document quality gate is 3.`);
    if (canonicalCore.size < 3)
        reasons.push(`Only ${canonicalCore.size}/32 Tier 1 health-impact biomarkers detected; minimum quality gate is 3.`);
    if (!criticalPresent)
    ...
    if (requiredTier1Coverage < TIER_1_REQUIRED_COVERAGE) {
        reasons.push(`Validated ${validatedRequiredTier1.length}/${requiredTier1Count} report-scope Tier 1 biomarkers; minimum required coverage is ${TIER_1_REQUIRED_COVERAGE}.`);
    }
    if (criticalValidationFailures.length > 0)
        reasons.push('Critical health-impact biomarker validation failed.');
    if (conflicts.length > 0 && criticalValidationFailures.length > 0)
        reasons.push('Conflicting Tier 1 biomarker values require manual review.');
    const rescanRequired = parameters.length < 3 ||
