...
export const buildExtractionGovernance = (text, parameters, document) => {
    ...
    const missingCriticalBiomarkers = ['HbA1c', 'Fasting Glucose', 'Hemoglobin', 'TSH', 'LDL Cholesterol'].filter((name) => !canonicalCore.has(name));
    ...
    if (parameters.length < 8)
        reasons.push(`Only ${parameters.length} biomarkers detected; minimum document quality gate is 8.`);
    ...
    if (validatedCore.size < MINIMUM_TIER_1_BIOMARKERS) {
        reasons.push(`Validated ${validatedCore.size}/${TIER_1_BIOMARKERS.length} Tier 1 health-impact biomarkers; minimum required coverage is ${MINIMUM_TIER_1_BIOMARKERS}/${TIER_1_BIOMARKERS.length}.`);
    }
