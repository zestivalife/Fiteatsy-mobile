...
import { acknowledgeMedicationExceptionForConsultant, getActiveMedicationExceptionsForOwner, getMedicationException, getMedicationExceptionsForOwner, getMedicationMonitoringForOwner } from '../medications/medications.service.js';
import { getConsultantLatestDietPlan, getConsultantNutritionIntelligence } from '../nutrition/nutrition.service.js';
import { getAssessmentResultById, getLatestAssessmentResult, listAssessmentResults } from '../assessments/assessments.repository.js';
...
import { ensureRegisteredClientsForEligibleUsers, getConsultantClientSyncDiagnostics, getConsultantWearableSummaryForClient, getRegisteredConsultantClientProfileContext, listAssignedConsultantClientContexts, listConsultantReportSummariesForClient, listConsultantTimelineForClient, listValidatedBiomarkerSummaryForClient, listRegisteredConsultantClients } from './consultants.repository.js';
...
export const canAccessConsultantClientApi = (account) => CONSULTANT_ROLES.has(account.user.role?.toLowerCase() ?? '');
const professionalTypeForAccount = (account) => account.user.role?.toLowerCase() === 'practitioner' ? 'PRACTITIONER' : account.user.role?.toLowerCase() === 'mentor' ? 'MENTOR' : 'CONSULTANT';
...
export const getConsultantClientProfile = async (publicClientId, account) => {
...
const availableValue = (metrics, type) => {
    const metric = metrics[type];
    return metric.status === 'AVAILABLE' ? metric.value : null;
};
const unavailableReason = (metrics, type) => {
    const metric = metrics[type];
...
const buildCompleteness = (profile) => {
    if (!profile) {
        return {
...
const foodPreferenceStatus = (foodPreferences) => {
    if (!foodPreferences?.updatedAtISO)
        return 'NOT_SET';
    const hasValues = Object.values(foodPreferences.profile).some((value) => Array.isArray(value) ? value.length > 0 : value != null);
    return hasValues ? 'COMPLETE' : 'INCOMPLETE';
};
const buildRecommendations = ({ completeness, reportsCount, wearableConnected, healthScore }) => {
...
const freshnessFromTimestamp = (value) => {
    if (!value)
        return 'no_data';
    const deltaMs = Date.now() - new Date(value).getTime();
    if (!Number.isFinite(deltaMs))
        return 'no_data';
    if (deltaMs > 1000 * 60 * 60 * 24 * 7)
        return 'stale';
    return 'fresh';
};
const buildConsentValidation = (account) => ({
    status: 'granted',
...
const buildAssignmentValidation = ({ account, healthProfile, careCase, cap003Assigned = false }) => {
...
const mapMedicationExceptionForConsultant = (exception, client) => ({
    id: exception.id,
...
const buildPlanWorkflow = ({ careCase, reportsCount, wearableConnected }) => {
...
const buildProvenance = ({ healthProfile, nutritionProfile, reports, biomarkers, wearableSummary, timeline, healthScores, careCase, lastSyncedAt }) => {
    ...
    const sources = [
        ...
        },
        {
            ...
            freshness: freshnessFromTimestamp(biomarkers[0]?.testDate ? new Date(biomarkers[0].testDate).toISOString() : null),
            fields: [
                'biomarkerId',
                'canonicalMarkerName',
                'rawMarkerName',
...
const resolveAssignedClientMedicationAccess = async (publicClientId, account) => {
    await ensureRegisteredClientsForEligibleUsers();
    const context = await getRegisteredConsultantClientProfileContext(publicClientId, account.accountId, professionalTypeForAccount(account));
    if (!context)
        return null;
...
const resolveAssignedClientAssessmentAccess = async (publicClientId, account) => {
    await ensureRegisteredClientsForEligibleUsers();
    const context = await getRegisteredConsultantClientProfileContext(publicClientId, account.accountId, professionalTypeForAccount(account));
    if (!context)
        return null;
...
const assessmentChange = (latest, previous) => latest && previous ? latest.rawScore - previous.rawScore : null;
const consultantAssessmentAccess = (account, assignmentValidation) => ({
    requestAccountId: account.accountId,
    requestRole: account.user.role ?? null,
    assignmentValidation,
    readOnly: true
});
export const getConsultantClientAssessmentSummary = async (publicClientId, account) => {
    const access = await resolveAssignedClientAssessmentAccess(publicClientId, account);
    if (!access)
        return null;
    if (!access.allowed) {
        return { error: 'CLIENT_ASSESSMENT_ACCESS_DENIED', assignmentValidation: access.assignmentValidation };
    }
...
export const getConsultantClientAssessmentResult = async (publicClientId, resultId, account) => {
    const access = await resolveAssignedClientAssessmentAccess(publicClientId, account);
    if (!access)
        return null;
    if (!access.allowed) {
    ...
    const result = await getAssessmentResultById({ accountId: access.context.accountId, clientId: access.context.internalClientId }, resultId);
    return result
        ? {
            client: access.context.profile.client,
            access: consultantAssessmentAccess(account, access.assignmentValidation),
...
export const getConsultantClientMedicationMonitoring = async (publicClientId, account) => {
    const access = await resolveAssignedClientMedicationAccess(publicClientId, account);
    if (!access)
        return null;
    if (access.status === 'forbidden') {
        return {
            error: 'CLIENT_MEDICATION_ACCESS_DENIED',
...
export const listConsultantMedicationExceptions = async (account) => {
    await ensureRegisteredClientsForEligibleUsers();
    const assignedClients = await listAssignedConsultantClientContexts(account.accountId);
    const exceptionsByClient = await Promise.all(assignedClients.map(async (client) => {
    ...
    const exceptions = exceptionsByClient
        .flatMap(({ client, exceptions: clientExceptions }) => clientExceptions.map((exception) => mapMedicationExceptionForConsultant(exception, {
    ...
    const byType = exceptions.reduce((counts, exception) => {
        counts[exception.type] = (counts[exception.type] ?? 0) + 1;
        return counts;
    }, {});
    const bySeverity = exceptions.reduce((counts, exception) => {
        counts[exception.severity] = (counts[exception.severity] ?? 0) + 1;
        return counts;
    }, {});
    const byStatus = exceptions.reduce((counts, exception) => {
...
export const getConsultantClientMedicationExceptions = async (publicClientId, account) => {
    const access = await resolveAssignedClientMedicationAccess(publicClientId, account);
    if (!access)
        return null;
    if (access.status === 'forbidden') {
...
export const getConsultantMedicationExceptionDetail = async (exceptionId, account) => {
    const exception = await getMedicationException(exceptionId);
    if (!exception)
        return null;
    const assignedClients = await listAssignedConsultantClientContexts(account.accountId);
    const matchedClient = assignedClients.find((client) => client.internalClientId === exception.clientId && client.accountId === exception.userId);
    if (!matchedClient) {
        return {
            error: 'CLIENT_MEDICATION_ACCESS_DENIED'
        };
...
export const acknowledgeConsultantMedicationException = async (exceptionId, account) => {
...
export const getConsultantClientWorkspace = async (publicClientId, account) => {
    await ensureRegisteredClientsForEligibleUsers();
    const context = await getRegisteredConsultantClientProfileContext(publicClientId, account.accountId, professionalTypeForAccount(account));
    if (!context)
        return null;
