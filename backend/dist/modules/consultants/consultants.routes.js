...
import { acknowledgeConsultantMedicationException, canAccessConsultantClientApi, getConsultantClientMedicationExceptions, getConsultantClientMedicationMonitoring, getConsultantClientAssessmentResult, getConsultantClientAssessmentSummary, getConsultantClientWorkspace, getConsultantMedicationExceptionDetail, getConsultantClientProfile, listConsultantMedicationExceptions, listConsultantClients } from './consultants.service.js';
...
const sendAssessmentAccessResponse = (res, payload) => {
    if (!payload) {
        return res.status(404).json({
            error: 'CLIENT_NOT_FOUND',
            message: 'Client not found or not available for consultant management.'
