...
const audit = async (input) => {
    await pool.query(`insert into qa_provisioning_audit_events
      (id, actor_user_id, target_user_id, assignment_id, action, account_purpose, role, reason, metadata)
     values ($1, $2, $3, $4, $5, $6, $7, $8, $9::jsonb)`, [crypto.randomUUID(), input.actorUserId, input.targetUserId ?? null, input.assignmentId ?? null,
...
export const recordQaIdentityReuse = async (input) => audit({
...
export const provisionQaIdentity = async (input) => {
    ...
    try {
        ...
        await client.query(`select pg_advisory_xact_lock(hashtextextended($1, 0))`, [`qa_identity:${normalizedEmail}:${normalizedMobileNumber}`]);
        const existing = await client.query(`select id, name, email_normalized, mobile_number_normalized, role, status, account_purpose, created_at
         from users
        where deleted_at is null
          and (email_normalized = $1 or mobile_number_normalized = $2)
        for update`, [normalizedEmail, normalizedMobileNumber]);
       ...
       returning id, name, email_normalized, mobile_number_normalized, role, status, account_purpose, created_at`, [crypto.randomUUID(), input.name.trim(), normalizedEmail, normalizedMobileNumber, input.role]);
        ...
        const clientRecord = input.role === 'user' ? await createOrResolveClientForAccount(user.id) : null;
        if (clientRecord) {
            const owner = { accountId: user.id, clientId: clientRecord.id };
...
export const getQaIdentity = async (userId) => {
    const result = await pool.query(`select id, name, email_normalized, mobile_number_normalized, role, status, account_purpose, created_at
       from users where id = $1 and deleted_at is null and account_purpose = 'QA_TEST'`, [userId]);
    return result.rowCount ? mapUser(result.rows[0]) : null;
};
export const isQaFixtureEntity = async (fixtureSetId, entityId, entityType = 'USER') => {
...
export const listQaAssignments = async () => {
    const result = await pool.query(`select a.id, a.consultant_user_id, a.client_user_id, a.status, a.scope, a.starts_at, a.ends_at, a.created_at, a.updated_at,
...
export const deactivateQaIdentity = async (input) => {
    const result = await pool.query(`update users set status = 'disabled', updated_at = now(), version = version + 1
    ...
    return { userId: input.userId, status: 'disabled' };
};
export const issueQaSessionAudit = async (actorUserId, targetUserId, reason) => {
    await audit({ actorUserId, targetUserId, action: 'QASessionIssued', accountPurpose: 'QA_TEST', reason });
};
export const resetQaOnboarding = async (input) => {
    const client = await pool.connect();
    try {
        await client.query('begin');
        const target = await client.query(`select u.id, hp.id as health_profile_id
         from users u
        ...
        for update of u, hp`, [input.userId]);
        if (!target.rowCount) {
            await client.query('rollback');
            return null;
        }
        const healthProfileId = String(target.rows[0].health_profile_id);
        await client.query(`update health_profiles
        ...
        where id = $1`, [healthProfileId]);
        await client.query(`update nutrition_profiles
          set completion_percent = 0, readiness_score = 0, ai_ready = false,
              missing_fields = '[]'::jsonb, section_scores = '[]'::jsonb,
              updated_at = now(), version = version + 1
        where health_profile_id = $1 and deleted_at is null and status = 'active'`, [healthProfileId]);
        await client.query(`update recovery_programs
          set current_phase = 'health_profile_pending', updated_at = now(), version = version + 1
        where health_profile_id = $1 and deleted_at is null and status = 'active'`, [healthProfileId]);
        await client.query(`update care_cases
        ...
        return { userId: input.userId, healthProfileId, onboardingStatus: 'INCOMPLETE' };
    }
    catch (error) {
        await client.query('rollback');
        throw error;
...
export const createQaAssignment = async (input) => {
    const result = await pool.query(`insert into consultant_client_assignments
     ...
     returning id, consultant_user_id, client_user_id, status, scope, created_at`, [crypto.randomUUID(), input.consultantUserId, input.clientUserId, input.actorUserId]);
    if (!result.rowCount)
        return null;
    const assignment = result.rows[0];
    await pool.query(`update health_profiles hp set assigned_consultant_id = $1, updated_at = now(), version = hp.version + 1
       from fiteatsy_clients c where c.account_user_id = hp.user_id and hp.user_id = $2 and hp.deleted_at is null`, [input.consultantUserId, input.clientUserId]);
    await pool.query(`update care_cases cc set assigned_consultant_id = $1, updated_at = now(), version = version + 1
       where cc.user_id = $2 and cc.deleted_at is null`, [input.consultantUserId, input.clientUserId]);
...
export const revokeQaAssignment = async (input) => {
    const result = await pool.query(`update consultant_client_assignments set status = 'revoked', ends_at = now(), updated_at = now()
       where id = $1 and status = 'active' returning *`, [input.assignmentId]);
    if (!result.rowCount)
        return null;
    const assignment = result.rows[0];
    await pool.query(`update health_profiles hp set assigned_consultant_id = null, updated_at = now(), version = version + 1
       where hp.user_id = $1 and hp.assigned_consultant_id = $2 and hp.deleted_at is null`, [assignment.client_user_id, assignment.consultant_user_id]);
    await pool.query(`update care_cases cc set assigned_consultant_id = null, updated_at = now(), version = version + 1
       where cc.user_id = $1 and cc.assigned_consultant_id = $2 and cc.deleted_at is null`, [assignment.client_user_id, assignment.consultant_user_id]);
