...
const roleAssignmentSchema = z.object({
    role: z.preprocess((value) => (typeof value === 'string' ? value.toLowerCase() : value), z.enum(['user', 'consultant', 'admin'])),
...
const adminError = (res, error) => { if (error instanceof CommonFoodAdminError)
    return res.status(error.statusCode).json({ error: error.code, message: error.message }); throw error; };
adminRouter.get('/common-foods', async (req, res) => { const parsed = z.object({ active: z.enum(['true', 'false']).transform(x => x === 'true').optional(), category: z.string().optional(), family: z.string().optional(), sourcePolicy: z.string().optional(), generatorEligible: z.enum(['true', 'false']).transform(x => x === 'true').optional(), mealHead: z.string().optional() }).safeParse(req.query); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT' }); try {
    return res.json(await listAdminCommonFoods(getAuthenticatedAccount(req), parsed.data));
}
catch (e) {
    return adminError(res, e);
} });
adminRouter.get('/common-foods/:foodId', async (req, res) => { try {
    return res.json(await inspectAdminCommonFood(getAuthenticatedAccount(req), req.params.foodId));
}
catch (e) {
    return adminError(res, e);
} });
adminRouter.patch('/common-foods/:foodId/state', async (req, res) => { const parsed = z.object({ active: z.boolean().optional(), generatorEligible: z.boolean().optional(), reason: adminReason }).refine(x => x.active !== undefined || x.generatorEligible !== undefined).safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT' }); try {
    return res.json(await setAdminCommonFoodState(getAuthenticatedAccount(req), req.params.foodId, parsed.data));
}
catch (e) {
    return adminError(res, e);
} });
adminRouter.post('/common-foods/:foodId/aliases', async (req, res) => { const parsed = z.object({ alias: z.string().trim().min(1), reason: adminReason }).safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT' }); try {
    return res.status(201).json(await addAdminCommonFoodAlias(getAuthenticatedAccount(req), req.params.foodId, parsed.data));
}
catch (e) {
    return adminError(res, e);
} });
adminRouter.delete('/common-foods/:foodId/aliases/:aliasId', async (req, res) => { const parsed = z.object({ reason: adminReason }).safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT' }); try {
    return res.json(await removeAdminCommonFoodAlias(getAuthenticatedAccount(req), req.params.foodId, req.params.aliasId, parsed.data.reason));
}
catch (e) {
    return adminError(res, e);
} });
adminRouter.post('/common-foods/:foodId/servings', async (req, res) => { const parsed = z.object({ displayName: z.string().min(1), grams: z.number().positive(), minMultiplier: z.number().positive(), maxMultiplier: z.number().positive(), allowedMultipliers: z.array(z.number().positive()).min(1), reason: adminReason }).safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT' }); try {
    return res.status(201).json(await addAdminCommonFoodServing(getAuthenticatedAccount(req), req.params.foodId, parsed.data));
}
catch (e) {
    return adminError(res, e);
} });
adminRouter.patch('/common-foods/:foodId/servings/:servingId/state', async (req, res) => { const parsed = z.object({ active: z.boolean(), reason: adminReason }).safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT' }); try {
    return res.json(await setAdminCommonFoodServingState(getAuthenticatedAccount(req), req.params.foodId, req.params.servingId, parsed.data));
}
catch (e) {
    return adminError(res, e);
} });
adminRouter.get('/common-foods/:foodId/audit', async (req, res) => { try {
    return res.json(await listAdminCommonFoodAudit(getAuthenticatedAccount(req), req.params.foodId));
}
catch (e) {
    return adminError(res, e);
} });
adminRouter.put('/common-foods/:foodId/meal-eligibility', async (req, res) => { const parsed = z.object({ mealHead: z.string().min(1), eligible: z.boolean(), reason: adminReason }).safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT' }); try {
    return res.json(await setAdminCommonFoodMeal(getAuthenticatedAccount(req), req.params.foodId, parsed.data));
}
catch (e) {
    return adminError(res, e);
} });
adminRouter.put('/common-foods/:foodId/roles', async (req, res) => { const parsed = z.object({ role: z.string().min(1), enabled: z.boolean(), reason: adminReason }).safeParse(req.body); if (!parsed.success)
    return res.status(400).json({ error: 'INVALID_INPUT' }); try {
    return res.json(await setAdminCommonFoodRole(getAuthenticatedAccount(req), req.params.foodId, parsed.data));
}
catch (e) {
    return adminError(res, e);
} });
...
adminRouter.get('/qa-identities/:userId', async (req, res) => {
    try {
        ...
        if (!identity)
            return res.status(404).json({ error: 'QA_IDENTITY_NOT_FOUND' });
...
adminRouter.post('/qa-identities/:userId/onboarding/reset', async (req, res) => {
    ...
    try {
        ...
        if (!result)
            return res.status(404).json({ error: 'QA_CLIENT_NOT_FOUND' });
        return res.status(200).json(result);
    }
    catch (error) {
        const typed = error;
...
adminRouter.post('/client-assignments', async (req, res) => {
    ...
    try {
        ...
        if (!result)
            return res.status(400).json({ error: 'INVALID_QA_ASSIGNMENT', message: 'Both identities must be active QA accounts with compatible roles.' });
