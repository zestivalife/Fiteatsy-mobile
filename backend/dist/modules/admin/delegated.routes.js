...
const actorId = (req) => {
    const value = req.delegatedAuthority?.sub;
    if (!value)
        throw Object.assign(new Error('Delegated actor is missing.'), { status: 401, code: 'INVALID_ACTOR' });
    return value;
};
...
const correlationReason = (req, reason) => `${reason} [correlation:${req.header('x-correlation-id') || 'unavailable'}]`;
delegatedRouter.post('/qa-clients', requireDelegatedAuthority('fiteatsy.qa.identity.create', 'qa_provisioning'), async (req, res) => {
    ...
    try {
        ...
        return respondError(res, error, 'QA_CLIENT_PROVISIONING_FAILED');
    }
});
delegatedRouter.post('/qa-consultants', requireDelegatedAuthority('fiteatsy.qa.identity.create', 'qa_provisioning'), async (req, res) => {
    ...
    try {
        ...
        return respondError(res, error, 'QA_CONSULTANT_PROVISIONING_FAILED');
    }
});
delegatedRouter.post('/qa-senior-consultants', requireDelegatedAuthority('fiteatsy.qa.identity.create', 'qa_provisioning'), async (req, res) => {
    ...
    try {
        ...
        return respondError(res, error, 'QA_SENIOR_CONSULTANT_PROVISIONING_FAILED');
    }
});
delegatedRouter.post('/qa-admins', requireDelegatedAuthority('fiteatsy.qa.admin.create', 'qa_provisioning', 'platform_owner'), async (req, res) => {
    ...
    if (!idempotencyKey)
        return res.status(400).json({ error: 'IDEMPOTENCY_KEY_REQUIRED', message: 'A non-empty idempotency key is required.' });
    ...
    try {
        ...
        return respondError(res, error, 'QA_ADMIN_PROVISIONING_FAILED');
    }
});
delegatedRouter.post('/client-assignments', requireDelegatedAuthority('fiteatsy.client.assign', 'client_assignment'), async (req, res) => {
    ...
    try {
        const result = await executeDelegatedIdempotently({ operation: 'client_assignment', key: req.header('idempotency-key') || '', execute: async () => {
                ...
                if (!assignment)
                    throw Object.assign(new Error('Both identities must be active QA accounts with compatible roles.'), { status: 400, code: 'INVALID_QA_ASSIGNMENT' });
                return { assignment };
            } });
        return res.status(result.replayed ? 200 : 201).json({ ...result.value, idempotentReplay: result.replayed });
    }
    catch (error) {
        return respondError(res, error, 'QA_ASSIGNMENT_FAILED');
    }
});
delegatedRouter.delete('/client-assignments/:assignmentId', requireDelegatedAuthority('fiteatsy.client.assignment.revoke', 'client_assignment'), async (req, res) => {
    try {
        ...
        if (!result)
            return res.status(404).json({ error: 'ASSIGNMENT_NOT_FOUND', message: 'Assignment was not found or is already revoked.' });
        return res.status(200).json({ assignment: result });
    }
    catch (error) {
        return respondError(res, error, 'QA_ASSIGNMENT_REVOKE_FAILED');
    }
});
delegatedRouter.post('/qa-identities/:userId/deactivate', requireDelegatedAuthority('fiteatsy.qa.identity.deactivate', 'qa_provisioning'), async (req, res) => {
    ...
    try {
        ...
        if (!result)
            return res.status(404).json({ error: 'QA_IDENTITY_NOT_FOUND', message: 'Active QA identity was not found.' });
        return res.status(200).json(result);
    }
    catch (error) {
        return respondError(res, error, 'QA_DEACTIVATION_FAILED');
    }
});
delegatedRouter.post('/qa-identities/:userId/onboarding/reset', requireDelegatedAuthority('fiteatsy.qa.onboarding.reset', 'qa_provisioning'), async (req, res) => {
    ...
    try {
        ...
        if (!result)
            return res.status(404).json({ error: 'QA_CLIENT_NOT_FOUND', message: 'Active QA client was not found.' });
        return res.status(200).json(result);
    }
    catch (error) {
        return respondError(res, error, 'QA_ONBOARDING_RESET_FAILED');
    }
});
