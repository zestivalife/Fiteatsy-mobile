...
const mapCareCase = (row) => ({
    ...
    previousStage: row.previous_stage == null ? null : String(row.previous_stage),
...
const createRecoveryProgramIfMissing = async (healthProfileId, stage) => {
...
export const getCareCaseById = async (careCaseId) => {
    const result = await pool.query(`
      select *
      from care_cases
      where id = $1
...
export const listTimelineEvents = async (careCaseId) => {
    const result = await pool.query(`
      select *
...
export const createHealthTicket = async (input) => {
    const createdAtISO = nowIso();
    const inserted = await pool.query(`
...
export const listHealthTickets = async (careCaseId) => {
    const result = await pool.query(`
      select *
      from health_tickets
      where care_case_id = $1
...
export const updateHealthTicket = async (ticketId, patch) => {
    const result = await pool.query(`
      select *
      from health_tickets
      where id = $1
