...
export class DestructiveTestResetBlockedError extends Error {
    code = 'DESTRUCTIVE_TEST_RESET_BLOCKED';
...
const parseDatabaseTarget = (databaseUrl, environment) => {
    let parsed;
    try {
...
export const assertDestructiveTestResetAllowed = (source = process.env) => {
    ...
    if (PRODUCTION_NAME_PATTERN.test(hostname)
