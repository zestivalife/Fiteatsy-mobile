...
const shouldSkipLegacySnapshotOnFreshReplay = async (client, file, startedWithEmptyLedger) => {
    if (!startedWithEmptyLedger || file !== LEGACY_PROFESSIONAL_SNAPSHOT_FILE)
        return false;
    const result = await client.query(`select count(*)::int as matching_count
       from users
      where id = any($1::text[])`, [LEGACY_PROFESSIONAL_SNAPSHOT_USER_IDS]);
...
const applyMigrations = async () => {
    ...
    try {
        ...
        for (const file of files) {
            ...
            try {
                await ensureSchemaMigrationsTable(client);
                const skipLegacySnapshot = await shouldSkipLegacySnapshotOnFreshReplay(client, file, startedWithEmptyLedger);
                if (!skipLegacySnapshot)
