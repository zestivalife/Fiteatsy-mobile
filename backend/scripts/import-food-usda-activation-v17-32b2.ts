...
import { createUsdaActivationV1732B2Records, FOOD_USDA_ACTIVATION_V17_32B2_ARTIFACT_SHA256 } from '../src/modules/nutrition/food-usda-activation-v17-32b2.js';
...
const main = async () => {
  ...
  try {
    ...
    await client.query('select pg_advisory_xact_lock($1,$2)', [20260907, 32]);
    for (const record of records) {
      const result = await client.query(
        `insert into food_catalogue_usda_activation_v17_32b2 (
        ...
        where food_catalogue_usda_activation_v17_32b2.activation_payload <> excluded.activation_payload
           or food_catalogue_usda_activation_v17_32b2.artifact_sha256 <> excluded.artifact_sha256`,
        [
          record.activationId,
          ...
          FOOD_USDA_ACTIVATION_V17_32B2_ARTIFACT_SHA256,
      ...
      for (const eventType of record.auditEvents) {
        const audit = await client.query(
          `insert into food_catalogue_usda_activation_audit_v17_32b2
          ...
          [crypto.randomUUID(), record.activationId, record.referenceItemId, eventType, record.processorVersion, JSON.stringify({ artifactSha256: FOOD_USDA_ACTIVATION_V17_32B2_ARTIFACT_SHA256, selectedFdcId: record.selectedFdcId })]
        );
    ...
    process.stdout.write(`${JSON.stringify({
      ...
      auditInserted,
      auditUnchanged,
      newGovernedMappings: records.filter((record) => record.activationType === 'NEW_MAPPING').length,
      ...
      artifactSha256: FOOD_USDA_ACTIVATION_V17_32B2_ARTIFACT_SHA256
    })}\n`);
  } catch (error) {
