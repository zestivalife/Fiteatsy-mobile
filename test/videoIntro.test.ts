...
describe('premium app-level video intro contract', () => {
  ...
  it('enforces the timeout, fallback, cleanup, and reduced-motion contracts', () => {
    expect(source).toContain('SPLASH_MAX_DURATION_MS');
    expect(source).toContain('SPLASH_MIN_DURATION_MS');
    expect(source).toContain("player.addListener('statusChange'");
    expect(source).toContain("player.addListener('playToEnd'");
    expect(source).not.toContain('requestExit(');
    expect(source).toContain('shouldExitSplash');
