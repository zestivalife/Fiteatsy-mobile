...
const isFoodPreferenceProfileComplete = (profile: FoodPreferenceProfile | null) => {
  if (!profile) return false;
  return Boolean(
    profile.dietType ||
    profile.staplePreference ||
    profile.dairyPreference ||
    profile.proteins.length ||
    profile.cuisines.length ||
    profile.foodsLiked.length ||
    profile.foodsDisliked.length ||
    profile.foodsAvoided.length ||
    profile.restrictions.length ||
    profile.practicality.length,
  );
};
