...
export const NutritionExperienceScreen = () => {
  ...
  const loadRecommendations = React.useCallback(async (
    ...
    date: string,
    mealKey?: string,
    selection?: { cuisine?: string; craving?: string },
  ) => {
    if (!mode) return;
    ...
    try {
      if (mode === 'what-can-eat-now') {
        ...
        setRecommendations(await getEatingOutSuggestions({ mealKey, date, cuisine: selection?.cuisine ?? activeCuisine }));
      } else {
        setRecommendations(await getCravingSuggestions({ mealKey, date, craving: selection?.craving ?? craving }));
      }
    } catch (e) {
  ...
  const updateMeal = async (meal: NutritionMeal, state: 'PENDING' | 'CONSUMED_APPROVED' | 'SKIPPED', option?: NutritionMeal['options'][number]) => {
    if (!data) return;
    setBusy(true);
  ...
  return <Screen scroll contentStyle={styles.screen}>
    ...
    {tab === 'pattern' ? <Pattern pattern={pattern} /> : <>
      ...
      <QuickActions
        onOpenRecommendation={isToday && hasRecommendationContext ? () => openRecommendation('what-can-eat-now') : undefined}
        onOpenEatingOut={isToday && hasRecommendationContext ? () => openRecommendation('eating-out') : undefined}
        onOpenCraving={isToday && hasRecommendationContext ? () => openRecommendation('craving') : undefined}
      />
      <Balance data={data} />
      <PlannedActual data={data} />
      <Adherence data={data} />
      <Text style={styles.section}>TODAY'S MEALS</Text>
      {data.meals.map(meal => <Meal key={meal.key} meal={meal} busy={busy || !isToday} onChoose={() => { setPendingChoiceId(meal.selectedOptionId); setSelected(meal); }} onLog={(option) => void updateMeal(meal, 'CONSUMED_APPROVED', option)} onSkip={() => void updateMeal(meal, 'SKIPPED')} />)}
      <Modal visible={selected != null} transparent animationType="slide" onRequestClose={() => setSelected(null)}><View style={styles.backdrop}><View style={styles.modal}><Text style={styles.modalTitle}>Choose your {selected?.label}</Text><Text style={styles.muted}>Choose 1 of 5 Consultant-approved options</Text><ScrollView contentContainerStyle={styles.options}>{selected?.options.map(option => { const checked = pendingChoiceId === option.id; return <Pressable accessibilityRole="radio" accessibilityState={{ checked }} key={`${selected.key}-${option.id ?? option.slot}`} disabled={busy} onPress={() => setPendingChoiceId(option.id ?? null)} style={[styles.option, checked && styles.optionSelected]}><View style={styles.between}><Text style={[styles.optionName, styles.flex]}>{option.meal}</Text><Ionicons name={checked ? 'radio-button-on' : 'radio-button-off'} size={20} color={checked ? C.blue : C.muted} /></View><Text style={styles.muted}>Serving: {option.portion}</Text><Text style={styles.nutrients}>{option.approxKcal ?? '—'} kcal · P {option.proteinGrams ?? '—'}g · C {option.carbsGrams ?? '—'}g · F {option.fatGrams ?? '—'}g</Text></Pressable>; })}</ScrollView><Pressable disabled={busy || !pendingChoiceId || !selected} onPress={() => { const option = selected?.options.find((item) => item.id === pendingChoiceId); if (selected && option) void updateMeal(selected, 'PENDING', option); }} style={[styles.primary, (!pendingChoiceId || busy) && styles.disabled]}><Text style={styles.primaryText}>Use this meal</Text></Pressable><Pressable onPress={() => setSelected(null)} style={styles.cancel}><Text style={styles.body}>Cancel</Text></Pressable></View></View></Modal>
      ...
      <RecommendationModal
        ...
        onCuisine={(cuisine) => {
          ...
          void loadRecommendations('eating-out', data.selectedDate, undefined, { cuisine });
        }}
        onCraving={(type) => {
          setCraving(type);
          void loadRecommendations('craving', data.selectedDate, undefined, { craving: type });
        }}
...
const QuickActions = ({ onOpenRecommendation, onOpenEatingOut, onOpenCraving }: {
  onOpenRecommendation?: () => void;
  onOpenEatingOut?: () => void;
  onOpenCraving?: () => void;
}) => <View style={styles.quickActions}>
  <QuickAction label="What can I eat now?" icon="add-circle-outline" color={C.green} onPress={onOpenRecommendation} />
  <QuickAction label="Eating Out" icon="calendar-outline" color={C.purple} onPress={onOpenEatingOut} />
  <QuickAction label="Craving something?" icon="bulb-outline" color={C.yellow} onPress={onOpenCraving} />
</View>;

const QuickAction = ({ label, icon, color, onPress }: { label: string; icon: React.ComponentProps<typeof Ionicons>['name']; color: string; onPress?: () => void }) => <Pressable accessibilityRole="button" accessibilityLabel={label} disabled={!onPress} onPress={onPress} style={[styles.quickActionCard, !onPress && styles.disabled]}><View style={[styles.quickActionIcon, { backgroundColor: `${color}14` }]}><Ionicons name={icon} size={25} color={color} /></View><Text style={styles.quickActionText}>{label}</Text></Pressable>;

const Balance = ({ data }: { data: NutritionExperience }) => {
  const needs = [['protein', data.remaining.protein], ['fibre', data.remaining.fibre]].filter(([, v]) => typeof v === 'number' && v > 0).map(([name]) => name);
  return <View style={styles.balance}><View style={styles.balanceIcon}><Ionicons name="git-compare-outline" size={25} color={C.green} /></View><View style={styles.flex}><Text style={styles.balanceTitle}>Balance My Day</Text><Text style={styles.balanceBody}>{needs.length ? `${needs.join(' and ')} still ${needs.length > 1 ? 'need' : 'needs'} attention for the rest of your day.` : 'Your logged nutrition is currently balanced against today’s plan.'}</Text>{data.meals.some(meal => meal.state === 'PENDING') ? <Text style={styles.balanceBody}>Your remaining meals are ranked against today’s actual intake and published targets.</Text> : null}</View></View>;
};
...
}) => <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}><View style={styles.backdrop}><View style={styles.modal}><Text style={styles.modalTitle}>{mode === 'what-can-eat-now' ? 'What can I eat now' : mode === 'eating-out' ? 'Eating Out' : 'Craving Suggestions'}</Text>{recommendations ? <Text style={styles.sheetContext}>{recommendations.mealLabel} · {recommendations.mealWindow}</Text> : null}{error ? <Text style={styles.errorText}>{error}</Text> : null}{mode === 'eating-out' ? <ScrollView horizontal style={styles.rowScroll} contentContainerStyle={styles.rowScrollContent}>{['north indian', 'south indian', 'chinese', 'continental', 'fast food'].map(cuisine => <Pressable key={cuisine} onPress={() => onCuisine(cuisine)} style={[styles.chip, activeCuisine === cuisine && styles.chipActive]}><Text style={styles.chipText}>{cuisine}</Text></Pressable>)}</ScrollView> : null}{mode === 'craving' ? <ScrollView horizontal style={styles.rowScroll} contentContainerStyle={styles.rowScrollContent}>{['sweet', 'salty', 'crunchy', 'spicy'].map(type => <Pressable key={type} onPress={() => onCraving(type)} style={[styles.chip, craving === type && styles.chipActive]}><Text style={styles.chipText}>{type}</Text></Pressable>)}</ScrollView> : null}{loading ? <ActivityIndicator color={C.blue} style={{ marginVertical: spacing.md }} /> : <ScrollView contentContainerStyle={styles.options}>{recommendations?.recommendations.length ? recommendations.recommendations.map(item => <Pressable key={`${item.slot}-${item.id ?? item.mealName}`} disabled={busy} onPress={() => onChoose(item, recommendations.mealKey)} style={styles.option}><View style={styles.between}><Text style={[styles.optionName, styles.flex]}>{item.mealName}</Text><Text style={[styles.sourceBadge, item.recommendationMode === 'approved' ? styles.approvedBadge : styles.guidanceBadge]}>{item.recommendationMode === 'approved' ? 'Approved' : 'Reviewed guidance'}</Text></View><Text style={styles.muted}>{item.portion}</Text><Text style={styles.nutrients}>{item.approxKcal ?? '—'} kcal · P {item.proteinGrams ?? '—'}g · C {item.carbsGrams ?? '—'}g · F {item.fatGrams ?? '—'}g · Fibre {item.fibreGrams ?? '—'}g</Text><Text style={styles.recommendationReason}>{item.nutritionRationale} · Match {item.rankingScore}</Text></Pressable>) : <View style={styles.emptyRecommendations}><Ionicons name="restaurant-outline" size={28} color={C.muted} /><Text style={styles.optionName}>{recommendations?.guidanceStatus === 'preparing' ? 'Guidance is being prepared' : 'No reviewed matches yet'}</Text><Text style={[styles.muted, styles.emptyText]}>{recommendations?.guidanceStatus === 'preparing' ? 'Additional food guidance is being prepared by your care team.' : 'Try another reviewed context. No unreviewed catalogue foods are shown.'}</Text></View>}</ScrollView>}<Pressable onPress={onClose} style={styles.cancel}><Text style={styles.body}>Close</Text></Pressable></View></View></Modal>;
...
const Meal = ({ meal, busy, onChoose, onLog, onSkip }: { meal: NutritionMeal; busy: boolean; onChoose: () => void; onLog: (option: NutritionMeal['options'][number]) => void; onSkip: () => void }) => {
  const [showComponents, setShowComponents] = React.useState(false);
  const option = meal.options.find((item) => item.id === meal.selectedOptionId) ?? (meal.state === 'CONSUMED_APPROVED' ? meal.options.find((item) => item.id === meal.consumed?.optionId) : undefined);
  const state = meal.state === 'PENDING' ? 'PENDING' : meal.state === 'SKIPPED' ? 'SKIPPED' : meal.state === 'CONSUMED_OUT_OF_PLAN' ? 'OUT OF PLAN' : 'CONSUMED';
  return <View style={[styles.card, styles.meal]}><View style={styles.between}><View><Text style={styles.mealTitle}>{meal.label}</Text><Text style={styles.muted}>{meal.window}</Text></View><Text style={[styles.status, meal.state === 'CONSUMED_APPROVED' && { color: C.green }, meal.state === 'CONSUMED_OUT_OF_PLAN' && { color: C.yellow }]}>{state}</Text></View>{option ? <><Text style={styles.optionName}>{option.meal}</Text><Text style={styles.muted}>Serving: {option.portion}</Text><Text style={styles.nutrients}>{option.approxKcal ?? '—'} kcal · P {option.proteinGrams ?? '—'}g · C {option.carbsGrams ?? '—'}g · F {option.fatGrams ?? '—'}g · Fibre {option.fibreGrams ?? '—'}g</Text>{option.components?.length ? <><Pressable onPress={() => setShowComponents((value) => !value)}><Text style={styles.blue}>{showComponents ? 'Hide ingredients' : 'View ingredients / quantities'}</Text></Pressable>{showComponents ? <View style={styles.componentList}>{option.components.map((component) => <Text key={component.id ?? `${component.name}-${component.serving}`} style={styles.muted}>• {component.name}: {component.serving}</Text>)}</View> : null}</> : null}</> : <Text style={styles.muted}>{meal.options.length ? 'Choose one of your Consultant-approved options.' : 'No approved options provided.'}</Text>}{meal.state === 'PENDING' ? <View style={styles.actions}><Pressable disabled={busy} onPress={onChoose} style={styles.primary}><Text style={styles.primaryText}>{option ? 'Change meal' : 'Choose meal'}</Text></Pressable>{option ? <Pressable disabled={busy} onPress={() => onLog(option)} style={styles.secondary}><Text style={styles.body}>Log meal</Text></Pressable> : null}<Pressable disabled={busy} onPress={onSkip} style={styles.secondary}><Text style={styles.body}>Skip</Text></Pressable></View> : meal.consumedAtISO ? <Text style={styles.muted}>Logged {new Date(meal.consumedAtISO).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</Text> : null}</View>;
};
...
const styles = StyleSheet.create({
  ...
  quickActions: { flexDirection: 'row', gap: spacing.xs }, quickActionCard: { flex: 1, minWidth: 0, minHeight: 132, borderRadius: radius.lg, borderWidth: 1, borderColor: C.line, backgroundColor: C.card, padding: spacing.sm, justifyContent: 'space-between', alignItems: 'flex-start' }, quickActionIcon: { width: 44, height: 44, borderRadius: radius.md, alignItems: 'center', justifyContent: 'center' }, quickActionText: { ...typography.bodyStrong, color: C.text, fontSize: 13, lineHeight: 18 },
  balance: { flexDirection: 'row', gap: spacing.sm, borderRadius: radius.lg, borderWidth: 1, borderColor: '#205936', backgroundColor: '#0B1B13', padding: spacing.md }, balanceIcon: { width: 36, height: 36, borderRadius: radius.md, backgroundColor: '#10331F', alignItems: 'center', justifyContent: 'center' }, balanceTitle: { ...typography.bodyStrong, color: C.text, fontSize: 15 }, balanceBody: { ...typography.caption, color: '#75B184', fontSize: 12, lineHeight: 18, marginTop: spacing.xs }, rowScroll: { marginVertical: spacing.sm }, rowScrollContent: { gap: spacing.xs, paddingBottom: spacing.sm }, chip: { borderRadius: radius.md, borderWidth: 1, borderColor: C.line, backgroundColor: C.raised, paddingHorizontal: spacing.sm, paddingVertical: spacing.xs }, chipActive: { borderColor: C.blue, backgroundColor: '#17364A' }, chipText: { ...typography.caption, color: C.text, textTransform: 'capitalize' }, uppercase: { textTransform: 'uppercase' }, sheetContext: { ...typography.caption, color: C.muted, marginTop: -spacing.xs }, sourceBadge: { ...typography.caption, fontFamily: 'Exo_600SemiBold', fontSize: 9, lineHeight: 12, textTransform: 'uppercase', borderRadius: radius.pill, paddingHorizontal: spacing.xs, paddingVertical: 4 }, approvedBadge: { color: C.green, backgroundColor: '#10271A' }, guidanceBadge: { color: C.yellow, backgroundColor: '#2D2410' }, recommendationReason: { ...typography.caption, color: C.blue, fontSize: 11, lineHeight: 15 }, emptyRecommendations: { alignItems: 'center', gap: spacing.xs, paddingVertical: spacing.xl }, emptyText: { textAlign: 'center', maxWidth: 260 },
