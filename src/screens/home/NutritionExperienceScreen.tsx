...
export const NutritionExperienceScreen = () => {
  ...
  const [serverToday, setServerToday] = React.useState<string>(isoDay(new Date()));
  ...
  const refresh = React.useCallback(async (date?: string) => {
    setError(null);
    try {
      const payload = await getNutritionExperience(date);
      setData(payload);
      if (!date) {
        setServerToday(payload.selectedDate);
        setSelectedDate(payload.selectedDate);
      }
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Unable to load Nutrition.');
    }
  }, []);
  ...
  const confirmRecommendation = async () => {
    ...
    try {
      ...
      setData(await logNutritionEvent(payload));
    ...
    <Screen scroll contentStyle={styles.screen}>
      <View style={styles.topRow}>
        <View style={styles.tabs}>
          <Tab label="Today's Plan" active={tab === 'today'} onPress={() => setTab('today')} />
          <Tab
            label="My Pattern"
            active={tab === 'pattern'}
            onPress={async () => {
              setTab('pattern');
              if (!pattern) setPattern(await getNutritionPattern(selectedDate));
            }}
          />
        </View>
        <Pressable
          accessibilityRole="button"
          accessibilityLabel="Select Nutrition date"
          onPress={() => { setDraftDate(new Date(`${selectedDate}T12:00:00`)); setShowCalendar(true); }}
      ...
      <Modal visible={showCalendar} transparent animationType="slide" onRequestClose={() => setShowCalendar(false)}>
        <Pressable style={styles.backdrop} onPress={() => setShowCalendar(false)}>
          <Pressable style={styles.pickerSheet} onPress={() => undefined}>
            <View style={styles.sheetHandle} />
            <View style={styles.sheetHeader}>
              <Pressable onPress={() => setShowCalendar(false)}><Text style={styles.sheetCancel}>Cancel</Text></Pressable>
              <Text style={styles.sheetTitle}>Select date</Text>
            <Pressable
              onPress={async () => {
                  const nextDate = isoDay(draftDate);
        ...
        <>
          {data.consultantNote ? (
            <View style={styles.consultant}>
              <View style={styles.info}><Ionicons name="information-circle-outline" size={25} color={C.blue} /></View>
              <View style={styles.flex}><Text style={styles.consultantLabel}>FROM YOUR CONSULTANT</Text><Text style={styles.consultantText}>{data.consultantNote}</Text></View>
            </View>
          ) : null}
          <View style={styles.planStrip}>
            <View style={styles.pill}><Text style={styles.pillText}>Active plan</Text></View>
            <Text style={styles.muted}>·</Text>
            <Text numberOfLines={1} style={[styles.muted, styles.flex]}>{data.version.content.nutritionSnapshot.programmeName}</Text>
            <Text style={styles.muted}>v{data.version.versionNumber}</Text>
          </View>
          <Summary data={data} />
          <View style={styles.twoCol}>
            <View style={[styles.card, styles.metric]}>
              <Text style={[styles.label, styles.blue]}>WATER</Text>
              <Text style={styles.value}>
                {data.water.litres.toFixed(1)} <Text style={styles.unit}>/ {data.water.targetLitres ?? '—'} L</Text>
              </Text>
              <Progress value={ratio(data.water.litres, data.water.targetLitres)} color={C.blue} />
              <Pressable
                disabled={busy || !isToday}
                onPress={() => { setWaterError(null); setShowWater(true); }}
                style={[styles.waterButton, !isToday && styles.disabled]}
              >
                <Text style={styles.waterText}>{isToday ? '+ Add water' : 'History is read-only'}</Text>
              </Pressable>
              {waterError ? <Text style={styles.errorText}>{waterError}</Text> : null}
            </View>
            <View style={[styles.card, styles.metric]}>
              <Text style={styles.label}>MEALS LOGGED</Text>
              <Text style={styles.value}>{data.mealsFollowed} <Text style={styles.unit}>of {data.mealCount}</Text></Text>
              <View style={styles.dots}>{data.meals.map(meal => <View key={meal.key} style={[styles.dot, { backgroundColor: meal.state === 'CONSUMED_APPROVED' ? C.green : meal.state === 'CONSUMED_OUT_OF_PLAN' || meal.state === 'SKIPPED' ? C.yellow : C.line }]} />)}</View>
              <Text style={styles.muted}>
                {pending} pending{data.skippedCount ? ` · ${data.skippedCount} skipped` : ''}{data.outOfPlanCount ? ` · ${data.outOfPlanCount} out of plan` : ''}
              </Text>
            </View>
          </View>
          <Balance
          ...
          {data.meals.map((meal) => (
            <Meal
              ...
              meal={meal}
              busy={busy || !isToday}
              onChoose={() => setSelected(meal)}
              onSkip={() => void updateMeal(meal, 'SKIPPED')}
            />
          ))}
        </>
      )}
      ...
      <Modal visible={selected != null} transparent animationType="slide" onRequestClose={() => setSelected(null)}>
        <View style={styles.backdrop}>
          <View style={styles.modal}>
            <Text style={styles.modalTitle}>Choose your {selected?.label}</Text>
            <Text style={styles.muted}>Consultant-approved options</Text>
            <ScrollView contentContainerStyle={styles.options}>
              {selected?.options.map(option => (
                <Pressable
                  key={`${selected.key}-${option.id ?? option.slot}`}
                  disabled={busy}
                  onPress={() => void updateMeal(selected, 'CONSUMED_APPROVED', option)}
                  style={styles.option}
                >
                  <Text style={styles.optionName}>{option.meal}</Text>
                  <Text style={styles.muted}>{option.portion} · {option.approxKcal ?? '—'} kcal · P {option.proteinGrams ?? '—'}g</Text>
                </Pressable>
              ))}
            </ScrollView>
            <Pressable onPress={() => setSelected(null)} style={styles.cancel}><Text style={styles.body}>Cancel</Text></Pressable>
          </View>
        </View>
      </Modal>

      <Modal visible={showWater} transparent animationType="slide" onRequestClose={() => setShowWater(false)}>
        <Pressable style={styles.backdrop} onPress={() => setShowWater(false)}>
          <Pressable style={styles.pickerSheet} onPress={() => undefined}>
            <View style={styles.sheetHandle} />
            <View style={styles.sheetHeader}>
              <Pressable onPress={() => setShowWater(false)}><Text style={styles.sheetCancel}>Cancel</Text></Pressable>
              <Text style={styles.sheetTitle}>Add water</Text>
              <View style={styles.sheetHeaderSpacer} />
            </View>
            <Text style={styles.waterPrompt}>How much water did you drink?</Text>
            <View style={styles.waterAmounts}>{[.25, .5, .75, 1].map(amount => <Pressable key={amount} onPress={() => setWaterAmount(amount)} style={[styles.waterAmount, waterAmount === amount && styles.waterAmountActive]}><Text style={[styles.waterAmountText, waterAmount === amount && styles.waterAmountTextActive]}>{amount < 1 ? `${amount * 1000} ml` : '1 L'}</Text></Pressable>)}</View>
            <Pressable disabled={busy} onPress={() => void saveWater()} style={[styles.saveWater, busy && styles.disabled]}>
              <Text style={styles.saveWaterText}>{busy ? 'Adding…' : 'Add water'}</Text>
            </Pressable>
          </Pressable>
        </Pressable>
      </Modal>

      <Modal visible={Boolean(recommendationMode)} transparent animationType="slide" onRequestClose={() => { setRecommendationMode(null); setRecommendations(null); }}>
        <View style={styles.backdrop}>
          <View style={styles.modal}>
            <Text style={styles.modalTitle}>
              {recommendationMode === 'what-can-eat-now'
                ? 'What can I eat now'
                : recommendationMode === 'eating-out'
                  ? 'Eating Out'
                  : 'Craving Suggestions'}
            </Text>
            {recommendationError ? <Text style={styles.errorText}>{recommendationError}</Text> : null}
            {recommendationMode === 'eating-out' ? (
              <ScrollView horizontal style={styles.rowScroll} contentContainerStyle={styles.rowScrollContent}>
                {['north indian', 'south indian', 'chinese', 'continental', 'fast food'].map(cuisine => (
                  <Pressable
                    key={cuisine}
                    onPress={() => {
                      setActiveCuisine(cuisine);
                      const currentDate = data?.selectedDate ?? selectedDate;
                      void loadRecommendations('eating-out', currentDate, recommendedMealKey);
                    }}
                    style={[styles.chip, activeCuisine === cuisine ? styles.chipActive : styles.chip]}
                  >
            ...
            {recommendationMode === 'craving' ? (
              <ScrollView horizontal style={styles.rowScroll} contentContainerStyle={styles.rowScrollContent}>
                {['sweet', 'salty', 'crunchy', 'spicy'].map(type => (
                  <Pressable
                    key={type}
                    onPress={() => {
                      setCraving(type);
                      const currentDate = data?.selectedDate ?? selectedDate;
                      void loadRecommendations('craving', currentDate, recommendedMealKey);
                    }}
                    style={[styles.chip, craving === type ? styles.chipActive : styles.chip]}
                  >
            ...
            {recommendationLoading ? (
              <ActivityIndicator color={C.blue} style={{ marginVertical: spacing.md }} />
            ) : (
              <ScrollView contentContainerStyle={styles.options}>
                {recommendations?.recommendations.map(item => (
                  <Pressable
                    key={`${item.slot}-${item.mealName}`}
                    disabled={busy}
                    onPress={() => void queueRecommendation(item, recommendations.mealKey)}
                    style={styles.option}
                  >
                    <View style={styles.between}>
                      <Text style={styles.optionName}>{item.mealName}</Text>
                      <Text style={[styles.muted, { textTransform: 'uppercase' }]}>
                        {item.recommendationMode === 'approved' ? 'Approved' : 'Out of plan'}
                      </Text>
                    </View>
                    <Text style={styles.muted}>
                      {item.portion} · {item.approxKcal ?? '—'} kcal · P {item.proteinGrams ?? '—'}g
                    </Text>
                  </Pressable>
                ))}
              </ScrollView>
            )}
            <Pressable onPress={() => { setRecommendationMode(null); setRecommendations(null); }} style={styles.cancel}><Text style={styles.body}>Close</Text></Pressable>
          </View>
        </View>
      </Modal>

      <Modal visible={Boolean(pendingRecommendation)} transparent animationType="slide" onRequestClose={() => setPendingRecommendation(null)}>
        <View style={styles.backdrop}>
          <View style={styles.pickerSheet}>
            <View style={styles.sheetHandle} />
            <Text style={styles.modalTitle}>Confirm logging</Text>
            <Text style={styles.optionName}>{pendingRecommendation?.item.mealName}</Text>
            <Text style={styles.muted}>Portion: {pendingRecommendation?.item.portion}</Text>
            <Text style={styles.muted}>Type: {pendingRecommendation?.item.recommendationMode === 'approved' ? 'Approved meal' : 'Out-of-plan'}</Text>
            <Text style={styles.muted}>Macros: kcal {pendingRecommendation?.item.approxKcal ?? '—'} / P {pendingRecommendation?.item.proteinGrams ?? '—'}g / C {pendingRecommendation?.item.carbsGrams ?? '—'}g / F {pendingRecommendation?.item.fatGrams ?? '—'}g / Fibre {pendingRecommendation?.item.fibreGrams ?? '—'}g</Text>
            <Pressable onPress={() => void confirmRecommendation()} style={[styles.primary, { marginTop: 12 }]}><Text style={styles.primaryText}>I ate this</Text></Pressable>
            <Pressable onPress={() => setPendingRecommendation(null)} style={[styles.cancel, { marginTop: 8 }]}><Text style={styles.body}>Cancel</Text></Pressable>
          </View>
        </View>
      </Modal>
    </Screen>
...
const Balance = ({ data, onOpenRecommendation, onOpenEatingOut, onOpenCraving }: {
  ...
  return <View style={styles.balance}>
    <View style={styles.balanceIcon}><Ionicons name="git-compare-outline" size={25} color={C.green} /></View>
    <View style={styles.flex}>
      <Text style={styles.balanceTitle}>Balance My Day</Text>
      <Text style={styles.balanceBody}>{needs.length ? `${needs.join(' and ')} still ${needs.length > 1 ? 'need' : 'needs'} attention for the rest of your day.` : 'Your logged nutrition is currently balanced against today’s plan.'}</Text>
      {data.meals.some(meal => meal.state === 'PENDING') ? <Text style={styles.balanceBody}>Your remaining approved meals have been ranked to help balance the rest of your day.</Text> : null}
      <View style={styles.recommendationQuickRow}>
        <Pressable disabled={!onOpenRecommendation} onPress={onOpenRecommendation} style={[styles.quickBtn, !onOpenRecommendation && styles.disabled]}><Text style={styles.quickBtnText}>What can I eat now</Text></Pressable>
        <Pressable disabled={!onOpenEatingOut} onPress={onOpenEatingOut} style={[styles.quickBtn, !onOpenEatingOut && styles.disabled]}><Text style={styles.quickBtnText}>Eating Out</Text></Pressable>
      </View>
      <Pressable disabled={!onOpenCraving} onPress={onOpenCraving} style={[styles.quickBtn, styles.quickBtnWide, !onOpenCraving && styles.disabled]}><Text style={styles.quickBtnText}>Craving</Text></Pressable>
    </View>
  </View>;
};
