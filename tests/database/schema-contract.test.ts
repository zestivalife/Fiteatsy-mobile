...
test('database schema defines traceable health intelligence scores', () => {
  ...
  assert.equal(hasPattern(/check \(score_type in \('energy_balance', 'body_support', 'nourishment', 'recovery', 'physical_wellness_index', 'active_performance', 'stress_resilience', 'nutrition', 'clinical', 'activity', 'sleep', 'calm', 'overall'\)\)/i), true);
});
