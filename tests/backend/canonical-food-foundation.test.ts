...
import { assertIndiaSourcePolicy, assertIngredientSourceState, assertMandatoryNutritionVector, assertSourceMappingEligible, assertSourceRights, canonicalHash, createValidationRelease, evaluateIndiaLocalLabEvidence, evaluateStageBReview, ingestHumanGateSubmission, ingredientIdentityKey, measureCoverage, servingVariant, type CanonicalIngredientIdentity, type IndiaLocalLabEvidence, type SourceMapping, type StageBReviewInput } from '../../backend/src/modules/nutrition/food-curation/canonical-food-foundation.js';
...
const v12Dir=new URL('../../backend/src/modules/nutrition/food-curation/data/',import.meta.url);
const v12Decisions=JSON.parse(fs.readFileSync(new URL('batch-1.stage-b-decisions.v12.json',v12Dir),'utf8'));
const v12Gate=JSON.parse(fs.readFileSync(new URL('batch-1.gate-report.v12.json',v12Dir),'utf8'));
const v12Release=(id:string)=>JSON.parse(fs.readFileSync(new URL(`batch-1.validation-release.${id}.v1.json`,v12Dir),'utf8'));
test('v12 accepts exactly three authorised Stage B approvals',()=>{assert.deepEqual(v12Decisions.decisions.map((x:any)=>x.recipeId),['CP_CHAPATI','CP_BHINDI_SABJI','CP_BHINDI_ALOO']);assert.ok(v12Decisions.decisions.every((x:any)=>x.decision==='APPROVED'&&x.reconciliationStatus==='PASS'))});
test('v12 never approves source-blocked Moong Dal or Peanut Poha',()=>{const ids=v12Decisions.decisions.map((x:any)=>x.recipeId);assert.ok(!ids.includes('CP_MOONG_DAL'));assert.ok(!ids.includes('CP_POHA_PEANUT'))});
test('v12 stale review-pack hash blocks release eligibility',()=>assert.ok(evaluateStageBReview({...stageB,expectedReviewPackHash:'expected'}).errors.includes('STALE_REVIEW_PACK_HASH')));
test('v12 missing reviewer authority blocks release eligibility',()=>assert.ok(evaluateStageBReview({...stageB,reviewerAuthority:''}).errors.includes('STAGE_B_REVIEWER_AUTHORITY_REQUIRED')));
test('v12 validation release factory creates a deeply immutable deterministic release',()=>{const input={validationReleaseId:'VR',recipeId:'R',recipeName:'Recipe',recipeVersion:'v1',validationReleaseVersion:1,stageADecisionId:'A',stageADecisionHash:'a',sourceDecisionIds:['S'],sourceDecisionHashes:['s'],canonicalIngredientMappings:[{ingredientId:'I',sourceRef:'SRC',sourceHash:'s'}],formulaHash:'f',measurementHash:'m',calculationHash:'c',reconciliationStatus:'PASS' as const,reconciliationHash:'r',stageBDecisionId:'B',stageBDecisionHash:'b',releaseStatus:'VALIDATED_RELEASED' as const,productionActivationStatus:'NOT_PRODUCTION_ACTIVE' as const,createdBy:'SYSTEM'};const a=createValidationRelease(input),b=createValidationRelease(structuredClone(input));assert.equal(a.releaseHash,b.releaseHash);assert.ok(Object.isFrozen(a)&&Object.isFrozen(a.canonicalIngredientMappings)&&Object.isFrozen(a.canonicalIngredientMappings[0]))});
test('v12 releases replay to their stored deterministic hashes',()=>{for(const id of ['CP_CHAPATI','CP_BHINDI_SABJI','CP_BHINDI_ALOO']){const r=v12Release(id);const payload=structuredClone(r);for(const k of ['schemaVersion','releaseTimestamp','immutable','releaseHash'])delete payload[k];assert.equal(canonicalHash(payload),r.releaseHash)}});
test('v12 released source snapshots survive later source supersession',()=>{const r=v12Release('CP_BHINDI_ALOO');const before=canonicalHash(r.canonicalIngredientMappings);const current={ingredientId:'POTATO',sourceRef:'FUTURE',sourceHash:'changed'};assert.notEqual(canonicalHash([...r.canonicalIngredientMappings,current]),before);assert.equal(canonicalHash(r.canonicalIngredientMappings),before)});
test('v12 represents partial 3/5 release and keeps population blocked',()=>{assert.equal(v12Gate.batchValidationStatus,'PARTIAL_3_OF_5_VALIDATION_RELEASED');assert.equal(v12Gate.indianFoodPopulationStatus,'BLOCKED_PENDING_BATCH_1_VALIDATION_COMPLETE')});
test('v12 production safety assertion keeps every release inactive',()=>{assert.equal(v12Gate.productionStatus,'UNCHANGED');assert.equal(v12Gate.dietPlanContract.totalOptions,35);assert.ok(v12Gate.recipes.every((x:any)=>x.productionActive===false));for(const id of ['CP_CHAPATI','CP_BHINDI_SABJI','CP_BHINDI_ALOO'])assert.equal(v12Release(id).productionActivationStatus,'NOT_PRODUCTION_ACTIVE')});
...
const v14Policy=JSON.parse(fs.readFileSync(new URL('food-source-policy.india.v1.json',v12Dir),'utf8'));
const v14Gate=JSON.parse(fs.readFileSync(new URL('batch-1.gate-report.v14.json',v12Dir),'utf8'));
for(const [jurisdiction,ingredient] of [['NEPAL','POHA'],['PAKISTAN','POHA'],['BANGLADESH','MOONG_DAL'],['AFRICA_REGIONAL','INDIAN_RECIPE']]) test(`v14 rejects ${jurisdiction} source for Indian-specific ${ingredient}`,()=>assert.throws(()=>assertIndiaSourcePolicy({sourceJurisdiction:jurisdiction,countryContext:'INDIA',isIndianSpecificFood:true,sourcePolicyClass:'FOREIGN_NATIONAL_DISALLOWED_FOR_INDIAN_FOOD'}),/FOREIGN_NATIONAL_SOURCE_REJECTED_FOR_INDIAN_CANONICAL_FOOD/));
test('v14 allows governed global generic USDA ingredients',()=>assert.doesNotThrow(()=>assertIndiaSourcePolicy({sourceJurisdiction:'UNITED_STATES',countryContext:'GLOBAL_GENERIC',isIndianSpecificFood:false,sourcePolicyClass:'GLOBAL_GENERIC_APPROVED'})));
test('v14 allows India-authoritative and India-local sources for Indian-specific Foods',()=>{for(const sourcePolicyClass of ['INDIA_AUTHORITATIVE','INDIA_LOCAL_LAB'] as const)assert.doesNotThrow(()=>assertIndiaSourcePolicy({sourceJurisdiction:'INDIA',countryContext:'INDIA',isIndianSpecificFood:true,sourcePolicyClass}))});
const validLab:IndiaLocalLabEvidence={canonicalIngredientId:'SPLIT_HULLED_YELLOW_MOONG_DAL',sampleCode:'LABREF_INDIA_SPLIT_HULLED_YELLOW_MOONG_DAL_B1',sampleCountry:'INDIA',sampleIdentityConfirmed:true,sampleIdentityHash:'s',chainOfCustodyComplete:true,chainOfCustodyHash:'c',labCountry:'INDIA',labId:'LAB',nablAccreditationNumber:'NABL-1',accreditationValid:true,foodAnalysisInScope:true,labReportHash:'r',reportMatchesSample:true,mandatoryVector:{energyKcal:1,proteinG:1,carbohydrateG:1,fatG:1,fibreG:1},methodsComplete:true,basis:'PER_100_G_EDIBLE',reviewerDecision:'APPROVED',reuseScope:'BATCH_1_RECIPE_REFERENCE'};
test('v14 complete governed Indian NABL evidence is deterministic and eligible',()=>{const a=evaluateIndiaLocalLabEvidence(validLab),b=evaluateIndiaLocalLabEvidence(structuredClone(validLab));assert.equal(a.eligible,true);assert.equal(a.state,'INDIAN_LOCAL_ANALYTICAL_VECTOR_VALIDATED');assert.equal(a.sourceHash,b.sourceHash)});
test('v14 foreign sample and foreign lab fail local-reference eligibility',()=>{const r=evaluateIndiaLocalLabEvidence({...validLab,sampleCountry:'NEPAL',labCountry:'UK'});assert.ok(r.errors.includes('INDIAN_SAMPLE_REQUIRED'));assert.ok(r.errors.includes('INDIA_BASED_LAB_REQUIRED'))});
test('v14 identity mismatch and incomplete custody fail local-reference eligibility',()=>{const r=evaluateIndiaLocalLabEvidence({...validLab,sampleIdentityConfirmed:false,chainOfCustodyComplete:false});assert.ok(r.errors.includes('INDIAN_SAMPLE_IDENTITY_NOT_ESTABLISHED'));assert.ok(r.errors.includes('CHAIN_OF_CUSTODY_INCOMPLETE'))});
test('v14 incomplete vector and invalid accreditation fail local-reference eligibility',()=>{const r=evaluateIndiaLocalLabEvidence({...validLab,accreditationValid:false,mandatoryVector:{...validLab.mandatoryVector,fibreG:null}});assert.ok(r.errors.includes('NABL_ACCREDITATION_INVALID_OR_OUT_OF_SCOPE'));assert.ok(r.errors.includes('MANDATORY_LAB_NUTRIENT_VECTOR_INCOMPLETE'))});
test('v14 policy and additive migration encode India-only enforcement',()=>{assert.deepEqual(v14Policy.allowedSourceClasses,['INDIA_AUTHORITATIVE','INDIA_LOCAL_LAB']);const sql=fs.readFileSync(new URL('../../backend/src/db/migrations/0048_india_food_source_policy.sql',import.meta.url),'utf8');for(const field of ['source_jurisdiction','country_context','is_indian_specific_food','source_policy_class','sample_country','lab_country'])assert.match(sql,new RegExp(field));assert.match(sql,/FOREIGN_NATIONAL_SOURCE_REJECTED_FOR_INDIAN_CANONICAL_FOOD/)});
test('v14 creates two exact, pending India lab request packages',()=>{for(const [file,id] of [['batch-1.india-lab-request.moong-dal.v1.json','SPLIT_HULLED_YELLOW_MOONG_DAL'],['batch-1.india-lab-request.dry-poha.v1.json','DRY_FLATTENED_RICE_POHA']]){const r=JSON.parse(fs.readFileSync(new URL(file,v12Dir),'utf8'));assert.equal(r.canonicalIngredientId,id);assert.equal(r.countryContext,'INDIA');assert.equal(r.state,'AWAITING_PHYSICAL_SAMPLE_AND_LAB');assert.equal(r.laboratory.country,'INDIA')}});
test('v14 gate remains truthful at 3/5 with no fabricated downstream evidence',()=>{assert.equal(v14Gate.indiaOnlySourcePolicy,'ENFORCED');assert.equal(v14Gate.foreignNationalFoodTablesActiveForIndianCanonicalFood,0);assert.deepEqual(v14Gate.counts,{validationReleases:3,newSourceMappings:0,newFormulaHashes:0,newCalculations:0,newStageBPacks:0});assert.equal(v14Gate.batchValidationStatus,'PARTIAL_3_OF_5_VALIDATION_RELEASED');assert.equal(v14Gate.productionStatus,'UNCHANGED')});
test('v14 preserves all existing release hashes and the 35-option Diet contract',()=>{for(const id of ['CP_CHAPATI','CP_BHINDI_SABJI','CP_BHINDI_ALOO']){const r=v12Release(id),payload=structuredClone(r);for(const k of ['schemaVersion','releaseTimestamp','immutable','releaseHash'])delete payload[k];assert.equal(canonicalHash(payload),r.releaseHash)}assert.deepEqual(v14Gate.dietPlanContract,{mealHeads:7,optionsPerMeal:5,totalOptions:35,lifecycleModified:false})});
