...
test('v17.23 catalogue scope and first-edit draft snapshots remain server governed',()=>{const service=readFileSync(new URL('../../backend/src/modules/nutrition/common-food-consultant.service.ts',import.meta.url),'utf8');const routes=readFileSync(new URL('../../backend/src/modules/nutrition/nutrition.routes.ts',import.meta.url),'utf8');assert.match(routes,/scope:z\.enum\(\['ALL','RECOMMENDED'\]\)/);assert.match(service,/mealEligibility:/);assert.match(service,/catalogEntityId:f\.id/);assert.match(routes,/draft:z\.object/);assert.match(service,/if\(!old&&!draft\)/);assert.match(service,/old\?\.components\?\?draft!/);});
